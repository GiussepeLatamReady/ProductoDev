...
||                                                              ||
||  File Name: LMRY_Fulfillment_Receipt_Populate_MPRD.js        ||
||                                                              ||
...
define(['N/config', 'N/email', 'N/log', 'N/url', 'N/record', 'N/search', 'N/runtime', 'N/file',
    ...
    '/SuiteBundles/Bundle 37714/Latam_Library/LMRY_UniversalSetting_Fulfillment_Receipt_LBRY',
    './EI_Library/LMRY_seriesObject_LBRY_v2.0',
    '/SuiteBundles/Bundle 37714/WTH_Library/LMRY_ItemReceiptTaxResults_LBRY_v2.0',
    '/SuiteBundles/Bundle 37714/Latam_Library/LMRY_Log_LBRY_V2.0'
],

    function (config, email, log, url, record, search, runtime, file, libraryEmail, libraryFeature, libraryHandler, libraryUS, librarySeries, libraryTaxReceipt, logLbry) {


        //245636 : DESARROLLO
        //243159 : QA
        var scriptObj = runtime.getCurrentScript();
        var LMRY_script = 'Latamready - Fulfillment Receipt Populate MPRD';

        var userId = scriptObj.getParameter({ name: 'custscript_lmry_ip_params_user_fillrcpt' });
        var stateId = scriptObj.getParameter({ name: 'custscript_lmry_ip_params_state_fillrcpt' });
        ...
        */
        function getInputData() {
            ...
            try {
                ...
                var fTranid = false;
                if (country == 'MEX') {
                    if (libraryFeature.getAuthorization(20, licenses) && libraryFeature.getAuthorization(314, licenses)) {
                        fTranid = true;
                    }
                } else if (country == 'BRA') {
                ...
                var fTaxReceiptBR = false;

                switch (country) {
                    case 'BRA':
                        fTaxReceiptBR = libraryFeature.getAuthorization(735, licenses);
                        break;
                }
                ...
                //SOLO MEXICO: PRE-IMPRESO POR LOTES LOCALIZATION
                if (country == 'MEX' && libraryFeature.getAuthorization(20, licenses)) {
                    ...
                    if (subsiOW) {
                        ...
                        log.debug('transactionContext 1', transactionContext);
                    } else {
                        var transactionContext = librarySeries.getSeries(transIDs, '', fPreimpreso.slice(1, fPreimpreso.length));
                        log.debug('transactionContext 2', transactionContext);
                    }
                ...
                for (var i = 0; i < idTransacciones.length - 1; i++) {
                    ...
                    if (country == 'MEX' && transactionContext['series'] && transactionContext['series'].length) {
        ...
        */
        function reduce(context) {
            ...
            try {
                ...
                var featurePreimpresoIF = currenData.values.fPreimpresoIF;
                var featurePreimpresoIR = currenData.values.fPreimpresoIR;
                ...
                var featureTaxReceiptBR = currenData.values.fTaxReceiptBR;
                ...
                if (search_transaction == 'ItemRcpt') {
                    type_transaction = 'itemreceipt';
                    featurePreimpresoTransaction = featurePreimpresoIR;
                    idTransaction = 16;
                }
                ...
                if (document_type != null && document_type != '') {
                    ...
                    if (serie_cxc != null && serie_cxc != '' && serie_cxc != -1 && featurePreimpresoTransaction == true) {
                        setearPreImpreso(id_load, country, feature_tranid, preImpresoLote);
                    }
                ...
                //Tax Receipt
                if (country == 'BRA' && featureTaxReceiptBR == true && seteoUS == true && type_transaction == 'itemreceipt') {
                    libraryTaxReceipt.setLinePosition(id_load);
                }
                ...
                if (country == 'BRA' && seteoUS == true && type_transaction == 'itemreceipt') {
                    if (featureTaxReceiptBR == true) {
                        libraryTaxReceipt.createItemReceiptTaxResults(id_load);
                    }
                }
                ...
                if (document_type != null && document_type != '') {
                    ...
                    if (feature_vip == true && sw_Continue) {
                        ...
                        var id_load4 = '';
                        if (country == 'PER' || country == 'MEX' || country == 'CHI') {
        ...
        */
        ...
        function validarPreimpreso(licenses, country) {
            ...
            //Localizacion - Transaction Number Item Fulfillment, TN Item Receipt
            var features = {
                'MX': [20, 426, 426],
                'BR': [140, 526, 734],
                'PE': [136, 68, 68]
        ...
        function setearPreImpreso(invoice, country, feature_tranid, preImpresoLote) {
        ...
        function status(country, arregloInvoice, subsidiary, isFact) {
            ...
            while (bandera) {
                ...
                if (country == 'MEX') {
                    var search_custbody = search.create({
                        type: 'transaction', columns: [columnas[posicion], 'internalid', search.createColumn({ name: "custrecord_lmry_ei_ds_restart", join: "CUSTRECORD_LMRY_EI_DS_DOC" })], filters: [{ name: 'internalid', operator: 'anyof', values: aux },
                        { name: 'mainline', operator: 'is', values: 'T' }, { name: 'type', operator: 'anyof', values: ['ItemShip'] }]
                    });
            ...
            email.send({ author: userId, recipients: employee_email, subject: 'LatamReady - Advanced Sales Flow', body: body });
