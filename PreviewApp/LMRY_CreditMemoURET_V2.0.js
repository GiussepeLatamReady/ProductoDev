...
define(['N/currency', 'N/log', 'N/config', 'N/ui/serverWidget', 'N/record', 'N/search', 'N/runtime',
    ...
    './Latam_Library/LMRY_MX_STE_Sales_Tax_Transaction_LBRY_V2.0', './Latam_Library/LMRY_CO_ST_Sales_Tax_Transaction_LBRY_V2.0',
    './Latam_Library/LMRY_PE_MapAndSaveFields_LBRY_v2.0', './Latam_Library/LMRY_CO_ST_CreditMemo_WHT_Lines_LBRY_V2.0',
    './Latam_Library/LMRY_CO_ST_WHT_Total_LBRY_V2.0', './WTH_Library/LMRY_TransferIva_LBRY',
    ...
    './Latam_Library/LMRY_libToolsFunctionsLBRY_V2.0', './WTH_Library/LMRY_BO_Taxes_LBRY_V2.0', './WTH_Library/LMRY_TransferIva_Subtype_LBRY'
],
    ...
    function (currency, log, config, serverWidget, record, search, runtime, libraryMail, library_HideView, libraryValidation, libraryNumber,
        ...
        MX_STE_TaxLibrary, CO_ST_TaxLibrary, PE_libMapTransactions, CO_ST_WhtLibrary_Lines, CO_ST_WhtLibrary_Total, libraryTransferIva,
        MX_ST_WhtLibrary_Total, AR_ST_TaxLibrary, LibraryValidatePeriod, library_PE_Detraction, AR_ST_PerceptionLibrary,
        AR_ST_TransFields, format, PA_ST_TaxLibrary, libraryTaxWithholding, library_BR_minimum, TaxWithholdingSales, libTools, libBoTaxes, LibraryTransferIvaSubtype) {
                        ...
                        switch (LMRY_Result[0]) {
                            ...
                            case "CO":
                                ...
                                if (libraryMail.getAuthorization(27, licenses) == true) {
                                    CO_ST_WhtLibrary_Total.deleteRelatedRecords(LMRY_Intern, "creditmemo");
                                }
                                break;
                            case "AR":
                        ...
                        switch (LMRY_Result[0]) {
                            ...
                            case "CO":
                                if (libraryMail.getAuthorization(645, licenses) == true) {
                                    CO_ST_TaxLibrary.setTaxTransaction(scriptContext);
                                }
                                if (libraryMail.getAuthorization(340, licenses) == true) {
                                    CO_ST_WhtLibrary_Lines.setWHTLineTransaction(scriptContext, licenses);
                                }
                                if (libraryMail.getAuthorization(27, licenses) == true) {
                                    CO_ST_WhtLibrary_Total.setWHTTotalTransaction(scriptContext, licenses);
                                }
                                break;
        ...
        function setBRDiscountAmount(recordObj) {
            ...
            if (numberItems) {
                var discountAmount = 0.00,
                    currentItemLine = -1, isGroupLine = false;
                ...
                if (idTrans != '' && idTrans != null) {
                    ...
                    if (invoiceID != '') {
                        ...
                        if (tranDate != "" && tranDate != null) {
                            var _TranDate = format.parse({ value: tranDate, type: format.Type.DATE })
        ...
        function Crear_Wht_Rule(recordObj, scriptContext) {
            ...
            if (countrysubsi == null) {
                ...
                var rulesSearchObj = search.create({
                    ...
                    columns:
                        [
                            ...
                            search.createColumn({ name: "custrecord_lmry_whtrule_country", label: "Latam - Country" }),
                            search.createColumn({ name: "internalid", label: "Internal ID" })
                        ]
                });
                //Se guardan los ids y names de los resultados  
                var resultRulesSearch = rulesSearchObj.run().getRange(0, 100);
                record_custpage.addSelectOption({ value: "0", text: "&nbsp;" });
                for (var i = 0; i < resultRulesSearch.length; i++) {
                    ...
                    log.debug({ title: idRS, details: nameRS });
                }
            ...
            for (var i = 0; i < numLines; i++) {
                var itemType = recordObj.getSublistValue({ sublistId: "item", fieldId: "itemtype", line: i }) || "";
                var value = recordObj.getSublistValue({ sublistId: "item", line: i, fieldId: "custcol_lmry_wht_rule" });
                value = Number(value) || "";
