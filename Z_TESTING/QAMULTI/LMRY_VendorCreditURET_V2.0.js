  ...
  function (record, serverWidget, search, runtime, log, config, library, library1, libraryDIOT,
    ...
    function beforeLoad(context) {
      try {
        ...
        if (type != 'print' && type != 'email') {
          ...
          if (LMRY_Result[0] == 'CO') {
            ...
            if (!library.getAuthorization(720, licenses) && !isSuiteAppInstalledCO) {
              ...
              var sublistExpense = formObj.getSublist({
                id: 'expense'
              });
              if (sublistExpense && JSON.stringify(sublistExpense) != '{}') {
                var expenseFieldCREE = sublistExpense.getField({
                  id: 'custcol_lmry_co_autoretecree'
                });
                if (expenseFieldCREE) {
                ...
                var expenseFieldIVA = sublistExpense.getField({
                  id: 'custcol_lmry_co_reteiva'
                });
                if (expenseFieldIVA) {
                ...
                var expenseFieldICA = sublistExpense.getField({
                  id: 'custcol_lmry_co_reteica'
                });
                if (expenseFieldICA) {
                ...
                var expenseFieldFTE = sublistExpense.getField({
                  id: 'custcol_lmry_co_retefte'
                });
                if (expenseFieldFTE) {
            ...
            Library_RenameFields.renameSublistFields(form, 'item', 'CO');
            Library_RenameFields.renameSublistFields(form, 'expense', 'CO');
        ...
        if (type_interface=='USERINTERFACE' && (type == 'create' || type == 'edit' || type == 'copy' || type == 'view') && LMRY_Result[0] == 'BO') {
          var useOnlyAtmainLevel = library.getAuthorization(46, licenses);
          BO_libWHTLines.setFieldWhtIVA(recordObj, form, type, useOnlyAtmainLevel);
        }
    ...
    function afterSubmit(context) {
      try {
        ...
        if (type == 'create' || type == 'edit') {
          ...
          if (LMRY_Result[0] == 'BO') {
            BO_libWHTLines.saveWhtIvaCode(LMRY_Intern,context.newRecord);
            if (library.getAuthorization(46, licenses)) {
              // Realiza la redireccion de cuentas
              BO_libWHTLines.createWHTbyLines(LMRY_Intern, 'vendorcredit', context.newRecord);
            }
          }
          if (LMRY_Result[0] == 'PY') {
            if (library.getAuthorization(47, licenses)) {
