 ...
 * @NModuleScope Public
 */
define(['N/log', "N/query", 'N/record', 'N/search', 'N/currentRecord', 'N/url', 'N/translation', 'N/https', 'N/runtime', './Latam_Library/LMRY_libSendingEmailsLBRY_V2.0',
  ...
  './WTH_Library/LMRY_WHT_Variable_Rate_popup_LBRY_V2.0'
],

  function (log, query, record, search, currentRecord, url, translation, https, runtime, library, library1, library2, library_ExchRate, libAlert, lbry_installment, lbry_valTranID, LR_webService, ST_ConfigFields, LbryBRDuplicate, libtools, kofaxLibrary, Library_ExchangeRate,Library_RedirecPayment,popupVariableRateLbry) {
    ...
    function pageInit(context) {
      try {
        ...
        if (!library.getAuthorization(117, licenses) && LMRY_countr[0] == 'CO') { //CO - Retenciones Manuales
        ...
        * Date  : 05/03/2020
        ...
        if (LMRY_countr[0] == 'CO'){
          ...
          const FEATURE_VARIABLE_RATE = popupVariableRateLbry.getFeatureVariableRate(subsidiaria);
          if (
            FEATURE_LOCALIZATION && 
            FEATURE_WHT_NEW_LINES_PURCHASE &&
            FEATURE_VARIABLE_RATE
          ) {
            if (["edit","copy","create"].indexOf(context.mode) != -1) {
              popupVariableRateLbry.loadDataVariableRate(context.currentRecord)
              popupVariableRateLbry.executePopup(context.currentRecord);
            }
          }
        }
        /**********************************************************************/
    ...
    function saveRecord(context) {
      ...
      try {
        ...
        /*Validacion formato 1005 y 1006*/
        var lineExpense = recordObj.getLineCount({ sublistId: "expense" });
        if (lineExpense > 0) {
          var msn = "";
          var alert01 = translation.get({
            collection: "custcollection_lmry_validation_process",
            key: "ACCOUNT_TAXCODE",
          })();
          for (var j = 0; j < lineExpense; j++) {
            var idAccount = recordObj.getSublistValue({ sublistId: "expense", fieldId: "account", line: j });
            if (validateMedioMagnetico(idAccount)) {
              var nameAccount = recordObj.getSublistValue({ sublistId: "expense", fieldId: "account_display", line: j });
              if (validateAccountTaxCode(idAccount)) {
                msn = msn + nameAccount + alert01 + "\n";
              }
            }
          }
          if (msn) {
        ...
        /* Fin validacion formato 1005 y 1006 */
        ...
        if (LMRY_countr[0] != 'PE' && LMRY_access && !notInstances.includes(instancia[0])) {
          ...
          if (existItemNeg.length > 0) {
            ...
            var notAllowInstans = ['5514786-sb1', '5514786', 'tstdrv1774174']
        ...
        ******************************************************************************/
        if (LMRY_countr[0] == 'AR') {
          if (library.getAuthorization(744, licenses)) {
            if (!libtools.validateTaxCode(recordObj)) {
              return false;
            }
          }
        }//*********************************************************
        ...
        ******************************************************************************/
        ...
        // Solo localizaciones Peruanas
        if (LMRY_access == true && LMRY_countr[0] == 'PE') {
    ...
    function validateField(context) {
      try {
         ...
         * Fecha : 2023.11.21
         ******************************* */
        if (context.fieldId == 'custbody_lmry_serie_doc_cxc' && LMRY_countr[0] == 'CR' && LMRY_countr[2] == true && LMRY_access == true) {
          recordObj.setValue({
            fieldId: 'custbody_lmry_serie_doc_cxp',
            value: recordObj.getText('custbody_lmry_serie_doc_cxc')
          });
        }
    ...
    function validateMedioMagnetico(accountsId) {
      try {
        var medioMagnetico = [];
        var boolean = false;
        var searchAccount = query.create({ type: "account" });
        /* Joins */
        var conceptoMagnetic = searchAccount.autoJoin({
          fieldId: "custrecord_lmry_co_puc_concept",
        });
        var formatoMagnetic = searchAccount.autoJoin({
          fieldId: "custrecord_lmry_co_puc_formatgy",
        });
        var conditionInactive = searchAccount.createCondition({
          fieldId: "isinactive",
          operator: query.Operator.IS,
          values: [false],
        });
        var conditionId = searchAccount.createCondition({
          fieldId: "id",
          operator: query.Operator.ANY_OF,
          values: accountsId,
        });
        searchAccount.condition = searchAccount.and(conditionInactive, conditionId);
        searchAccount.columns = [
          conceptoMagnetic.createColumn({
            fieldId: "custrecord_lmry_co_puc_formatid_c",
          }),
          formatoMagnetic.createColumn({
            fieldId: "custrecord_lmry_co_puc_number",
          }),
        ];
        var resultAccount = searchAccount.runPaged({
          pageSize: 50,
        });
        for (var i = 0; i < resultAccount.pageRanges.length; i++) {
          var page = resultAccount.fetch(i);
          var results = page.data.asMappedResults();
          results.forEach(function (scanresult) {
            medioMagnetico.push(scanresult.custrecord_lmry_co_puc_formatid_c);
            medioMagnetico.push(scanresult.custrecord_lmry_co_puc_number);
          });
        }
        if (medioMagnetico.includes("1005") || medioMagnetico.includes("1006")) {
          boolean = true;
        }
        return boolean;
      } catch (error) {
    ...
    function validateAccountTaxCode(idAcocunt) {
      try {
        var bool = false;
        var taxCodeSearch = search.create({
          type: "salestaxitem",
          columns: ["saleaccount", "purchaseaccount"],
          filters: [["purchaseaccount", "anyof", idAcocunt], "OR", ["saleaccount", "anyof", idAcocunt]],
        }).runPaged({ pageSize: 1000 });
        if (taxCodeSearch.count > 0) {
          bool = true;
        }
        return bool;
      } catch (error) {
