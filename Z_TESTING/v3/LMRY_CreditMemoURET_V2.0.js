...
define(['N/currency', 'N/log', 'N/config', 'N/ui/serverWidget', 'N/record', 'N/search', 'N/runtime',
  ...
  './Latam_Library/LMRY_Custom_ExchangeRate_Field_LBRY_V2.0.js', './Latam_Library/LMRY_Custom_ExchangeRate_LBRY_V2.0.js', "N/suiteAppInfo",
  ...
  function (currency, log, config, serverWidget, record, search, runtime, libraryMail, library_HideView, libraryValidation, libraryNumber,
    ...
    LibraryTransferIvaSubtype, PE_STE_TaxLibrary, CR_STE_WhtLibrary_Total, Library_ExchangeRate_Field, Library_ExchangeRate, suiteAppInfo,
    MXPedimentos
  ) {
      ...
      try {
        ...
        if (type != 'print' && type != 'email') {
          ...
          if (LMRY_Result[0] == 'CO') {
            ...
            if ((libraryMail.getAuthorization(340, licenses) || libraryMail.getAuthorization(721, licenses)) && !isSuiteAppInstalledCO) {
            ...
            if (!libraryMail.getAuthorization(721, licenses) && !isSuiteAppInstalledCO) {
              var formObj = scriptContext.form;
          ...
          //Boton UPDATE WHT
          if (LMRY_Result[0] == 'CO' && libraryMail.getAuthorization(721, licenses) && !isSuiteAppInstalledCO) {
          ...
          *******************************************************************************************************************************/
          ...
          if (LMRY_Result[0] == 'AR') {
            if (type_interface == 'USERINTERFACE') {
              Library_AutoPercepcionDesc.disabledSalesDiscount(form,subsidiary);
            }
          }

          if (LMRY_Result[0] == "CO" && ["copy", "create", "edit","view"].indexOf(type) != -1 && type_interface == "USERINTERFACE") {
            //Se crea el cust page            
            setOptionsOperationType(recordObj, form);
          }
    ...
    function beforeSubmit(scriptContext) {
      try {
        ...
        if (LMRY_Result[0] == "CO") {
          ...
          if (libraryMail.getAuthorization(721, licenses) == true) {
            if (scriptContext.type == 'edit') {
              var changeObj = Library_Duplicate.Verification_Duplicate(scriptContext, true);
        ...
        if ((["USERINTERFACE", "USEREVENT", "MAPREDUCE", "WEBSERVICES", "RESTWEBSERVICES", "RESTLET", "CSVIMPORT", "SUITELET"].indexOf(type_interface) != -1) && scriptContext.type == "create" && LMRY_Result[0] == 'AR') {
          libTools.inheritTaxes(recordObj);
        }

        if (LMRY_Result[0] == 'MX' && scriptContext.type == "delete") {
      ...
      try {
        ...
        ******************************************************/
        ...
        if (LMRY_Result[0] == "CO" && ["create", "edit", "copy"].indexOf(scriptContext.type) != -1) {
          ...
          setLineDiscount(recordObj);

        }

        if (["create", "edit", "copy"].indexOf(type) != -1 && type_interface == "USERINTERFACE") {
          var valueOperType = recordObj.getValue('custpage_lmry_operation_type');
          if (valueOperType != 0) {
            updateTransactionFieldCO(recordObj, valueOperType);
          }
        }
        ...
        if (LMRY_Result[0] === 'MX') {
          ...
          if (featPedimentos) {
            ...
            if (type == 'create' || type == 'edit') {
              ...
              var message = MXPedimentos.createPedimentos(recordObj.id, recordObj.id);
    ...
    /* ------------------------------------------------------------------------------------------------------
     * Seteo los campos de columna para el proceso de descuentos
     * --------------------------------------------------------------------------------------------------- */
    ...
    function setLineDiscount(newRecord) {
      try {
        ...
        libraryMail.sendemail2(' [ setLineDiscount ] ' + err, LMRY_script, newRecord, 'discount', 'transaction');
      }
    }
    ...
    function createCOLineFieldsWHT(formObj, recordObj, type) {
      try {
        ...
        var filters = [
          ...
          "AND",
          ["custrecord_lmry_ntax_transactiontypes", "anyof", "8"],
          "AND",
          ["custrecord_lmry_ntax_wht_tax_point", "anyof", ["@NONE@", "1"]]
        ];
        ...
        if (itemSublist && JSON.stringify(itemSublist) != "{}") {
          //RETECREE
          var itemFieldCREE = itemSublist.getField({
            id: 'custcol_lmry_co_autoretecree'
          });

          if (itemFieldCREE && JSON.stringify(itemFieldCREE) != "{}") {
          ...
          if (itemFieldFTE && JSON.stringify(itemFieldFTE) != "{}") {
          ...
          if (itemFieldICA && JSON.stringify(itemFieldICA) != "{}") {
          ...
          if (itemFieldIVA && JSON.stringify(itemFieldIVA) != "{}") {
    ...
    function setOptionsOperationType(recordObj, form) {
      var countrysubsi = recordObj.getValue({
        fieldId: 'custbody_lmry_subsidiary_country'
      });
      ...
      //Se crea el cust page
      var record_custpage = form.addField({
        id: "custpage_lmry_operation_type",
        label: 'Latam - CO Operation Type',
        type: serverWidget.FieldType.SELECT
      }).setHelpText("custpage_lmry_operation_type");

      //Se agrega al formulario
      form.insertField({ //Inserta un campo delante de otro campo.
        field: record_custpage,
        nextfield: "custbody_lmry_num_preimpreso"
      })

      var filters = new Array();
      filters[0] = search.createFilter({
        name: 'custrecord_lmry_operation_type_country',
        operator: search.Operator.ANYOF,
        values: [countrysubsi]
      });
      filters[1] = search.createFilter({
        name: 'custrecord_lmry_operation_type_ref',
      ...
      var operTypeSearchObj = search.create({
        type: "customrecord_lmry_operation_type",
        filters: filters,
        columns: ["name"]
      });
      var resultOperTypeSearch = operTypeSearchObj.run().getRange(0, 100);
      record_custpage.addSelectOption({
        value: "0",
        text: "&nbsp;"
      });
      for (var i = 0; i < resultOperTypeSearch.length; i++) {
        var idRS = resultOperTypeSearch[i].id;
        var nameRS = resultOperTypeSearch[i].getValue('name');
        //Se agregan las opciones a la seleccion del cust page
        record_custpage.addSelectOption({
          value: idRS,
          text: nameRS
        });
      }
      var idTrans = recordObj.id;
      if (idTrans) {
        var type = search.lookupFields({
          type: "transaction",
          id: idTrans,
          columns: ['custrecord_lmry_co_related_transaction.custrecord_lmry_co_operation_type']
        });
        var array = type['custrecord_lmry_co_related_transaction.custrecord_lmry_co_operation_type'];
        if (array.length > 0) {
          recordObj.setValue({
            fieldId: 'custpage_lmry_operation_type',
            value: array[0].value,
          });
        }
      }
    ...
    function updateTransactionFieldCO(recordObj, newValue) {
      var RecordID = recordObj.id;
      var searchCO = search.create({
        type: 'customrecord_lmry_co_transaction_fields',
        filters: [{
          name: 'custrecord_lmry_co_related_transaction',
          operator: 'is',
          values: RecordID
        }, {
          name: 'isinactive',
        ...
        columns: ['custrecord_lmry_co_operation_type']
      });
      searchCO = searchCO.run().getRange(0, 1);
      if (searchCO && searchCO.length) {
        var oldValue = searchCO[0].getValue('custrecord_lmry_co_operation_type');
        var coTransaction = record.load({
          type: 'customrecord_lmry_co_transaction_fields',
          id: searchCO[0].id
        });
        if (oldValue != newValue) {
          coTransaction.setValue('custrecord_lmry_co_related_transaction', RecordID);
          coTransaction.setValue('custrecord_lmry_co_operation_type', newValue);
        }
      } else {
        var coTransaction = record.create({
          type: 'customrecord_lmry_co_transaction_fields'
        });
        coTransaction.setValue('custrecord_lmry_co_related_transaction', RecordID);
        coTransaction.setValue('custrecord_lmry_co_operation_type', newValue);
