  ...
  function (currency, record, log, currentRecord, search, runtime, url, https, format, translation, libraryMail, library_Val, libraryNumber, library_ExchRate, library_translator,
      ...
      try {
        ...
        if (LMRY_countr[0] == 'CL' && libraryMail.getAuthorization(604, licenses) && (type == 'edit' || type == 'create' || type == 'copy')) {
          ...
          if (jsonCurrencies[currencyTransaction]['symbol'] == 'CLP' && fieldRateUF && recordObj.getField(fieldRateUF) && type != 'copy') {
            ...
            if (!rateUF && currencyUF) {

              var rateUF = currency.exchangeRate({ source: currencyUF, target: 'CLP', date: tranDate });
    ...
    function CMClnt_SaveRecord(scriptContext) {
      ...
      try {
        ...
        if (Object.keys(setuptaxValues).length > 0) {
          if (setuptaxValues["validateNegativeItem"] == true || setuptaxValues["validateNegativeItem"] == "T") {
    ...
    function CMClnt_ValidateField(scriptContext) {
      ...
      try {
        ...
        if (name == 'custpage_lmry_operation_type' && LMRY_access == true && boo_PageIni == false) {
          if (LMRY_countr[0] == 'CO') {
            var operType = recordObj.getValue('custpage_lmry_operation_type');            
            if (operType != '' && operType != null && operType != -1) {
              if (Val_Campos.length > 0) {
                library_Val.Val_Authorization(recordObj, LMRY_countr[0], licenses);
              }
            }
    ...
    function getSetupTaxSubsidiary(subsidiary) {
      ...
      var searchSetupTax = search.create({
          type: 'customrecord_lmry_setup_tax_subsidiary',
          columns: ['custrecord_lmry_setuptax_val_neg_item'],
          filters: [
              ["isinactive", "is", "F"]
          ]
      });
      ...
      if (subsi_OW == true || subsi_OW == "T") {
        searchSetupTax.filters.push(search.createFilter({ name: "custrecord_lmry_setuptax_subsidiary", operator: "anyof", values: subsidiary }))
      }
      searchSetupTax = searchSetupTax.run().getRange(0, 1);
      if (searchSetupTax.length > 0) {
          setuptaxObj = {
              "validateNegativeItem": searchSetupTax[0].getValue("custrecord_lmry_setuptax_val_neg_item")
          }
      }
      return setuptaxObj;
    }
