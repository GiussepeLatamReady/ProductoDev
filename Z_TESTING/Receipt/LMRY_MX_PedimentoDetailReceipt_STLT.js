...
||                                                              ||
||  File Name: LMRY_MX_PedimentoDetailReceipt_STLT.js           ||
||                                                              ||
 ...
 * @NModuleScope Public
 */
define(['N/record', 'N/log', 'N/xml', 'N/ui/serverWidget', 'N/search', 'N/runtime', 'N/format', './Latam_Library/LMRY_libSendingEmailsLBRY_V2.0'],

    function (record, log, xml, serverWidget, search, runtime, format, library) {
        // Nombre del Script
        var LMRY_script = "LatamReady - MX Pedimentos Detail Receipt";
        var FORM = '';
        var LANGUAGE = '';
        ...
        function onRequest(context) {
            try {
                ...
                var RECEIPT_ID = context.request.parameters.receiptid;
                ...
                if (context.request.method == 'GET') {
                    ...
                    var RCD_RECEIPT = search.lookupFields({
                        type: search.Type.ITEM_RECEIPT,
                        id: RECEIPT_ID,
                        columns: ['trandate', 'internalid', 'subsidiary', 'createdfrom', 'tranid']
                    });

                    var trandate = RCD_RECEIPT.trandate;
                    ...
                    if (subsi_OW == true) {
                        ...
                        p_subsi.addSelectOption({ value: RCD_RECEIPT.subsidiary[0].value, text: RCD_RECEIPT.subsidiary[0].text });
                    ...
                    p_source.addSelectOption({ value: RCD_RECEIPT.createdfrom[0].value, text: RCD_RECEIPT.createdfrom[0].text });
                    ...
                    p_trans.addSelectOption({ value: RECEIPT_ID, text: "Item Receipt #" + RCD_RECEIPT.tranid });
                    ...
                    //Número Pedimento
                    var pediment = SubTabla.addField({ id: 'id_pediment', label: 'PEDIMENTO', type: serverWidget.FieldType.TEXT }).updateDisplayType({ displayType: serverWidget.FieldDisplayType.ENTRY });
                    pediment.isMandatory = true;
                    ...
                    var Filter_Trans = search.createFilter({ name: 'internalid', operator: search.Operator.ANYOF, values: RECEIPT_ID });
                    ...
                    for (var i = 0; i < numItems; i++) {
                        ...
                        var trandate = format.format({
                            value: recordShipment.getValue("trandate"),
                        ...
                        if (inventoryDetail) {
                            ...
                            for (var j = 0; j < cLineas; j++) {
                                ...
                                if (Number(loteQuantity)) {
                                    SubTabla.setSublistValue({ id: 'custpage_quantity', line: index, value: loteQuantity });
                                } else {
                                    SubTabla.setSublistValue({ id: 'custpage_quantity', line: index, value: quantity });
                                }
                                index++;
                            }
                        } else {
                    ...
                    */
                    ...
                    // Script Cliente
                    FORM.clientScriptModulePath = './Latam_Library/LMRY_MX_PedimentosReceipt_CLNT.js';
        ...
        var TEXT_BY_LANG = {
            'title': { 'en': 'LatamReady - MX Pedimentos Detail Receipt', 'es': 'LatamReady - MX Detalle Pedimento Receipt', 'pt': 'LatamReady - MX Detalhe Pedimento Receipt' },
