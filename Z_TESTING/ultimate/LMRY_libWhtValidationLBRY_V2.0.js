...
define(['N/ui/serverWidget','./LMRY_libSendingEmailsLBRY_V2.0', './LMRY_libNumberInWordsLBRY_V2.0', 'N/log', 'N/record', 'N/search', 'N/runtime', 'N/format', './LMRY_CO_Duplicate_Credit_Memos_LBRY_V2.0','./LMRY_WhtValidattionEntity_LBRY_V2.0'],

    function (serverWidget,Library_Mail, Library_Number, log, record, search, runtime, format, Library_Duplicate, library_validation_entity) {
        ...
        function Create_WHT_Latam(Transaction, ID, context) {
            ...
            if (countryId == "29") {
                ...
                if (Transaction == 'vendorbill' || Transaction == 'vendorcredit') {
                    ...
                    var RETEBOIVA = Obj_RCD.getValue({ fieldId: 'custpage_lmry_bo_reteiva' });
                    if (RETEIU) array_wht.push(RETEIU);
                    if (RETEBOIVA) array_wht.push(RETEBOIVA);
                }
            }
        ...
        function saveRecordRelated(ID,dataWht,countryId){
            log.error("dataWht",dataWht);
            if (countryId == "29" && dataWht.label.slice(0,8) == "custpage") {
                log.error("validacion 1","bolivia y custpage");
                if (dataWht.local > 0) {
                    log.error("validacion 2","monto mayor que cero");
                    ...
                    record.submitFields({
                        ...
                        values: {
                            custrecord_lmry_bo_reteiva_whtamount: dataWht.local
                        },
                        options: { enableSourcing: true, ignoreMandatoryFields: true, disableTriggers: true }
                    });
                }
        ...
        function Search_WHT(Transaction, ID, Obj_RCD, WHTID, exchangeRate, setupTax) {
            try {
                ...
                for (var i = 0; i < objResult.length; i++) {
                    ...
                    log.debug("[variable_rate, amount_wht, base_wht, rate_wht, apply_transaction, surpass_minimum]", [variable_rate, amount_wht, base_wht, rate_wht, apply_transaction, surpass_minimum])
                    if (variable_rate && amount_wht && base_wht && rate_wht && apply_transaction && surpass_minimum) {
