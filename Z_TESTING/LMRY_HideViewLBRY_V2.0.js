...
define(['N/ui/serverWidget', 'N/search', 'N/log', './LMRY_libSendingEmailsLBRY_V2.0'],
  ...
  function(serverWidget, search, log, Library_Mail) {
    ...
    function validateRuc(countryCode,currentRCD) {
      try {
        // Solo para Peru
        var featureRuc = Library_Mail.getAuthorization(242, licenses);
        var documentTypeID = currentRCD.getValue('custentity_lmry_sunat_tipo_doc_id');
        if (countryCode != "PE" || !featureRuc) return false;
        if (!documentTypeID) {
