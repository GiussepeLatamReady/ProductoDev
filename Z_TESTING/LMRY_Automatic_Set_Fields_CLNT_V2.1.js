 ...
 * @NModuleScope Public
 */
define(['N/log', 'N/record', 'N/runtime', 'N/search', './Latam_Library/LMRY_libSendingEmailsLBRY_V2.0', './Latam_Library/LMRY_Log_LBRY_V2.0'],
    /**
     * @param{log} log
     * @param{record} record
     ...
     * @param{search} search
     */
    function (log, record, runtime, search, Library_Mail, Library_Log) {
        ...
        function pageInit(scriptContext) {
            try {
                ...
                if (['create', 'edit', 'copy'].includes(actionType)) {
                    ...
                    //Ocultar campo custpage
                    let customReasonField = recordObj.getField('custpage_shipping_reason');
                    if (actionType === 'create') {
                        console.log("customReasonField",customReasonField)
                        if (customReasonField) {
                            customReasonField.isVisible = false;
                        }
                        
                    } else {
                        let reasonField = recordObj.getField('custrecord_lmry_shipping_reason');
                        reasonField.isVisible = false;
                    }
                    if (customReasonField) {
                        addShippingReason(recordObj);
                    }
        ...
        function validateField(scriptContext) {
            try {
                ...
                if (fieldName === 'custpage_transaction') {
                    ...
                    if (Number(country) === 174) {
                        if (Number(transactionID) === 16) {
                            if (customReasonField) customReasonField.isVisible = true;
                            
                            reasonField.isVisible = false;
                        } else {
                            if (customReasonField) customReasonField.isVisible = false;
                        }
                    } else {
                        if (customReasonField) customReasonField.isVisible = false;
                    }
                ...
                if (fieldName === 'custrecord_set_service' || fieldName === 'custrecord_set_inventory' || fieldName == 'custrecord_set_bonus' || fieldName == 'custrecord_set_hibrid') {
                    ...
                    let arrayChecks = [check_Service, check_Inventory, check_Bonus, check_Hibrid];
                    ...
                    if (check_Single > 1) {
                        if (language == 'en') {
                            ...
                            alert('Solo puede ser servicio o inventario o bonificación o híbrido');
                        } else {
                            alert('Só pode ser servico ou inventário ou bonus ou híbrido');
                        }
        ...
        function saveRecord(scriptContext) {
            try {
                ...
                let typeTransaction = Number(recordObj.getValue('custrecord_lmry_us_transaction'));
                ...
                if (setupTax) {
                    ...
                    if (country == 11 && !Library_Mail.getAuthorization(323, licenses)) {
                        alert('AUTOMATIC FIELDS (A/R) feature is disabled');
                        return false;
                    }
                }
                ...
                if (country == 30) {
                    ...
                    if (typeTransaction === 7) {
                        ...
                        alert('Must select if it is inventory');
                        return false;
                    }

                }
                if (Number(country) === 174 && typeTransaction === 16) {
                    let custom_shipping_reason = recordObj.getValue('custpage_shipping_reason');
                    if (custom_shipping_reason) {
                        recordObj.setValue('custrecord_lmry_shipping_reason', custom_shipping_reason);
                    }
                    let shipping_reason = recordObj.getValue('custrecord_lmry_shipping_reason');
                    if (!custom_shipping_reason || !shipping_reason) {
                        alert('LATAM - SHIPPING REASON(SET) is required');
                        return false;
                    }
                }
        ...
        const replicatedData = (recordObj) => {
            ...
            if (transactionID == 5 || transactionID == 7 || transactionID == 10 || transactionID == 16 || transactionID == 17 || transactionID == 20 || transactionID == 32 || transactionID == 18) {
                filters.push(search.createFilter({
            ...
            let viewSearch = search.create({
                ...
                columns:
                    [
                        ...
                        "custrecord_lmry_setup_us_bill_payment"
                    ]
            });
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (!country) {
                    ...
                    let transactions = {
                        ...
                        billPayment: results[i].getValue("custrecord_lmry_setup_us_bill_payment") || false,
                    };
        ...
        const addShippingReason = (recordObj) => {
            let shipping_reason = recordObj.getValue('custrecord_lmry_shipping_reason');
            let shipping_reason_field = recordObj.getField({ fieldId: 'custpage_shipping_reason' });
            shipping_reason_field.removeSelectOption({ value: null });
            shipping_reason_field.insertSelectOption({ value: '', text: "&nbsp" });
            let dataShippingReason = search.create({
                type: 'customrecord_lmry_shipping_reason',
                filters: [
                    ['isinactive', 'is', 'F'], "AND",
                    ["formulatext: CASE WHEN {custrecord_lmry_moti_code} IN ('06', '07') THEN '1' ELSE '0' END","is","1"],  "AND", ['custrecord_lmry_moti_trans_by_country', 'is', '174']
                ],
                columns: ["name"]
            });
            dataShippingReason.run().each(function (result) {
                let id = result.id;
                let name = result.getValue('name');
                shipping_reason_field.insertSelectOption({ value: id, text: name });
                return true;
            });
            if (shipping_reason) {
                recordObj.setValue('custpage_shipping_reason', shipping_reason);
            }
        }
