    ...
    function (search,runtime) {
        ...
        const loadDataVariableRate = (currentRecord) => {
                ...
                const setDataLine = (sublist, fieldId, list) => {
                    ...
                    const items = list.reduce((items,item)=>{
                        if(item.lineuniquekey) items[item.lineuniquekey] = item;
                        return items
                    },{});
                    ...
                    for (let i = 0; i < countLines; i++) {
                        ...
                        const lineuniquekey = currentRecord.getCurrentSublistValue({ sublistId: sublist, fieldId: "lineuniquekey" });
                        console.log("value",items[lineuniquekey])
                        if (items[lineuniquekey]) {
                            currentRecord.setCurrentSublistValue({
                                sublistId: sublist,
                                fieldId: fieldId,
                                value: JSON.stringify(items[lineuniquekey])
                            });
                        }
        ...
        const executePopup = (currentRecord) => {
            ...
            const onClickVariableRate = (id,type) => {
              ...
              console.log("checkvariableRate",checkvariableRate)
        ...
        function createPopup(checkvariableRate, currentRecord, type) {
            ...
            const lineuniquekey = currentRecord.getCurrentSublistText({ sublistId: type, fieldId: "lineuniquekey" })
                    ...
                    buttons: [
                        {
                            ...
                            handler: function () {
                                ...
                                whtObject.lineuniquekey = lineuniquekey;
