...
define(['N/record', 'N/runtime', 'N/log', 'N/search', 'N/format', 'N/transaction', './LMRY_IP_libSendingEmailsLBRY_V2.0', 'N/query'],
    function (record, runtime, log, search, format, transaction, libraryEmail, query) {
            ...
            try {
                ...
                if (F_SUBSIDIAR == true || F_SUBSIDIAR == 'T') {
                    columns = [
                        ...
                        "accountingperiod.closed", "tranid", "fxamount", "custbody_lmry_num_preimpreso"
                    ];
                } else {
                    columns = [
                ...
                if ((!F_APPROVAL_INVOICE || F_APPROVAL_INVOICE == 'F') || ((F_APPROVAL_INVOICE == true || F_APPROVAL_INVOICE == 'T') && approvalstatus == 2)) {
                    ...
                    log.debug('[isClosedPeriod, F_REVERSALVOIDING]', [isClosedPeriod, F_REVERSALVOIDING].join(','));
                    ...
                    log.debug('whtObject', JSON.stringify(whtObject));
                    if (whtObject['id']) {
                        response['wht'] = true;
                    }
        ...
        function createVoidCreditMemo(id_invoice, invoice, idWHTInvoice, forms) {
            ...
            log.debug('creditMemo form', form);
            ...
            if (config && form) {
                ...
                if (country == 30) {
                    ...
                    // LR0011965 : Mejora por ISSUE Reportado
                    if (config['applyformat']=='T' || config['applyformat']==true){
                        // Seteando el campo Pre-Impreso + _DV
                        recordCreditMemo.setValue({
                            fieldId: 'custbody_lmry_num_preimpreso',
                            value: invoice.custbody_lmry_num_preimpreso + '_DV'
                        });    
                    }else{
                        // Seteando el campo memo del CM el tranid y el internal id del Invoice
                        recordCreditMemo.setValue({
                ...
                //log.debug('CheckBox de remover lines ', removeLines);
                ...
                log.debug('idCreditMemo', idCreditMemo);
            }
            return idCreditMemo;
        }
        ...
        function obtenerDatosConfigVoidTransaction(id_subsidiaria) {
            try {
                ...
                // LR0011965 se agrego el campo custrecord_lmry_configvoid_apply_format
                var columns = ['custrecord_lmry_configvoid_subsidiary', 'custrecord_lmry_configvoid_item',
                    ...
                    'custrecord_lmry_configvoid_apply_format'
                ];
                ...
                if (searchResult != '' && searchResult != null) {
                    ...
                    // Change Request 2024.10.07
                    config['applyformat'] = searchResult[0].getValue({
                        name: 'custrecord_lmry_configvoid_apply_format'
                    });
                    //  Segmentacion
        ...
        function createVoidWHTInvoice(idTransaction, whtObject, customSegments, forms) {
            try {
                ...
                log.debug('idWHTInvoice', idWHTInvoice);
                return idWHTInvoice;
            } catch (err) {
        ...
        function reversalJournal(recordId) {
            try {
                ...
                // log.debug("data custPymtAccountDetail", [custPymtAccountDetail[0], custPymtAccountDetail[1]]);
        ...
        function isVoidPaymentClosedPeriod(subsidiaryID){
            var isVoidPayment = false;
            if (!subsidiaryID) return false;
            search.create({
                type: "customrecord_lmry_setup_tax_subsidiary",
                columns: ['custrecord_lmry_sts_void_payment_cp'],
                filters: [
                  ['custrecord_lmry_setuptax_subsidiary', 'anyof', subsidiaryID], 'AND', ['isinactive', 'is', 'F']
                ]
            }).run().each(function(result){
                isVoidPayment = result.getValue("custrecord_lmry_sts_void_payment_cp");
            });

            return isVoidPayment
        }

        function reversalJournalClosedPeriod(recordId,paymentValues) {
            try {
                var result = {
                    trans: '',
                    fields: []
                ...
                var custPymtAccountDetail = query.runSuiteQL({
                    query: "SELECT BUILTIN.DF(TransactionAccountingLine.Account) AS Account, TransactionAccountingLine.Account, TransactionAccountingLine.Debit / Transaction.ExchangeRate AS DebitInTransactionCurrency, TransactionAccountingLine.Credit / Transaction.ExchangeRate AS CreditInTransactionCurrency, TransactionAccountingLine.Posting, TransactionLine.Memo, TransactionLine.id, TransactionLine.Transaction FROM accountingbook, TransactionLine, TransactionAccountingLine, Transaction WHERE TransactionLine.Transaction = TransactionAccountingLine.Transaction AND TransactionAccountingLine.Transaction = Transaction.ID AND accountingbook.isprimary = 'T' AND TransactionAccountingLine.accountingbook = accountingbook.id AND TransactionLine.id = '0' AND TransactionAccountingLine.Transaction = '" + recordId + "' AND (TransactionAccountingLine.Debit IS NOT NULL OR TransactionAccountingLine.Credit IS NOT NULL) ORDER BY TransactionLine.ID;" }).results;
