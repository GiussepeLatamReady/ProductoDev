...
processPerception("", "AR", "edit")
function processPerception(paramContext, CodeCountry, evento, isMassive, objTransaction) {
...
function calculatePerception(invoiceRecord, numLines, CodeCountry, event) {
    try {
        ...
        transaction.notSend = validateStatus(invoiceRecord.id)
        if (!transaction.notSend) {
...
function setLinePerceptionCreditMemo(transaction, invoice,setupTaxSubsidiary) {
    ...
    for (let i = 0; i < itemsInvoice.length; i++) {
...
function setLinePerception(appliesTo, recordTaxs, transaction, setupTaxSubsidiary, item, invoice) {
    if (recordTaxs.length) {
        ...
        for (var i = 0; i < recordTaxs.length; i++) {
            ...
            console.log("recordTax: ", recordTax)
            ...
            if (transaction.type.text == 'creditmemo' && JSON.stringify(invoice) != "{}") {
                ...
                var lineCount = 0;
                console.log("rowPercepction: ", rowPercepction)
                ...
                for (var lineuniquekey in invoice.items) {
                    ...
                    if (item.id == itemId && !item.inherited) {
                        console.log("item [creditmemo]: ", item)
...
function getItems(currentRecord) {
    ...
    for (var i = 0; i < lines; i++) {
        ...
        if (FeaDepa || FeaDepa == 'T'){
            items[key] = {
                department: currentRecord.getSublistValue({ sublistId: 'item', fieldId: 'department', line: i }),
            }
        }
        if (FeaClas || FeaClas == 'T'){
            items[key] = {
                class: currentRecord.getSublistValue({ sublistId: 'item', fieldId: 'class', line: i }),
            }
        }
        if (FeaLoca || FeaLoca == 'T'){
            items[key] = {
                location: currentRecord.getSublistValue({ sublistId: 'item', fieldId: 'location', line: i }),
            }
        }
    }
    return items;
