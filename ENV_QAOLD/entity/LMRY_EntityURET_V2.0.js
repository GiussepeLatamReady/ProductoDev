    ...
    function (search, runtime, serverWidget, log, Library_Mail, Library_HideView, library_Uni_Setting) {
        ...
        function beforeLoad(scriptContext) {
            try {
              ...
              if (isURET == 'view' && LMRY_countr[0] == 'BR' && LMRY_access == true) {
                log.error("entre a la validacion idEntity", idEntity)
                if (validatePagamentoLR(subsidiary)) {
                    FORM.getButton({
                        id: 'acceptpayment'
                    }).isHidden = true;
                    ...
                    FORM.addButton({
                        ...
                        label: 'Latam Payment', 
                        functionName: 'callModuleCustomerPayment(' + subsidiary +','+ idEntity +')',
                    });                    
                }                
              }
        ...
        function validatePagamentoLR(idSubsi) {
            var searchSetupTaxSubsi = search.create({
                type: "customrecord_lmry_setup_tax_subsidiary",
                filters: [["custrecord_lmry_setuptax_subsidiary", "anyof", idSubsi]],
                columns: ["custrecord_lmry_setuptax_br_pagamento_lr"]
            });

            var results = searchSetupTaxSubsi.run().getRange(0, 1);
            
            return results[0].getValue('custrecord_lmry_setuptax_br_pagamento_lr');
        }
