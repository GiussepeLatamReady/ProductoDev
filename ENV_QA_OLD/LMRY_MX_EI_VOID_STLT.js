...
define(['N/format', 'N/search', "N/email", "N/encode", 'N/runtime', 'N/record', 'N/file', "N/config", "N/url", "N/http", "N/https", "N/transaction", '/SuiteBundles/Bundle 243159/EI_Library/LMRY_AnulacionInvoice_LBRY_V2.0', 'SuiteBundles/Bundle 243159/EI_Send_Email_Module/LMRY_Transaction_Message_LBRY_V2.0.js', 'SuiteBundles/Bundle 243159/EI_Library/LMRY_EI_libSendingEmailsLBRY_V2.0'],
    function(format, search, email, encode, runtime, record, file, config, url, http, https, transaction, library_AnulacionInvoice, message_lbry, ei_library) {
        ...
        function onRequest(context) {
            ...
            if (context.request.method == 'GET') {
                ...
                try {
                    ...
                    log.debug('Valores', nInternalid + '-' + nTypeRec);
                    ...
                    var returnCancelar = WSCancelarXML(jsonEnableFeat);
                    ...
                    if (flagContemplar) {
                        log.debug("Error", mensajeError);
                        return true;
                    }
                    sleep(5000);
                    var returnConsultaCFE = WSConsultaCFE(jsonEnableFeat);
                    ...
                    log.debug("returnConsultaCFE:", returnConsultaCFE);
                    var returnDescargarXML = "";

                    if (exito) {
                        var descargarXML = WSDescargarXML(jsonEnableFeat);
                    ...
                    log.debug('Status 1:', status_doc.toString().substring(0, 300));
                    ...
                    sendMail(jsonEnableFeat, '', returnCancelar, returnDescargarXML, returnConsultaCFE, PDF);

                    /* ************** Codigo para el FileCabinet ************* */
                    if (exito) {
                        printPDFandXML();
                        if (status_doc == 'Cancelado') {
                            log.debug("Voided by NetSuite", nTypeRec);
                            var void_feature = runtime.getCurrentScript().getParameter({
                                name: 'REVERSALVOIDING'
                            });
                            if (nInternalid != '' && nInternalid != null) {
                                if (nTypeRec == 'invoice') {
                                    if (jsonEnableFeat.voidOnlyCurrent && (void_feature == false || void_feature == 'F') && !isCurrentMonth(nInternalid, nTypeRec)) {
                                        log.debug('[VOID ONLY CURRENT PERIOD]', 'The configuration for only voiding transactions within the current period is up')
                                        exito = 'Parcial';
                                        mensajeError = "Legally canceled successfully (But not in NetSuite) through LATAM - MX VOID ONLY CURRENT PERIOD active. You must void the transacction manually.";
                                        logError(status_doc, estatus, mensajeError);
                                    } else {
                                        if (!jsonEnableFeat.voidOnlyTransaction.includes("invoice")) {
                                            var resp_lbry = library_AnulacionInvoice.anularInvoice(nInternalid);
                                            log.debug("salidaLBRY", resp_lbry);

                                            if (resp_lbry.wht) mensajeLBRY = "El invoice tiene retenciones.";
                                            else mensajeLBRY = "El invoice NO tiene retenciones.";
                                            log.debug("wht", mensajeLBRY);

                                            if (resp_lbry.standardvoid) mensajeLBRY = "Se anuló de forma estandar.";
                                            else mensajeLBRY = "Se anula creando Credit Memo.";
                                            log.debug("standardvoid", mensajeLBRY);
                                            ...
                                            log.debug("idcreditmemo", mensajeLBRY);
                                    ...
                                    if (nTypeRec == 'customerpayment' || nTypeRec == 'customtransaction_lmry_payment_complemnt') {
                                        if (codModificacion != '01') {
                                            Remove_Trans(nInternalid, nTypeRec, void_feature, jsonEnableFeat);
                                        }
                                    } else {
                                        exito = 'Parcial';
                                        mensajeError = "Legally canceled successfully but not in NetSuite. You must void the transacction manually.";
                                ...
                                log.debug('internalIDDs', internalIDDs);
                                log.debug('el exito Final', exito)
                                if (exito == 'Parcial') {
                                    ...
                                    // log.debug('docProcStatus', docProcStatus);
                    ...
                    if (ei_library.getAuthorization("573", licencias_mx)) {
                        if ((status_doc == 'Cancelando' || status_doc == 'Cancelado') && jsonEnableFeat.sendEmailCancel) {
        ...
        function WSCancelarXML(jsonEnableFeat) {
            nombreWS = "WSCancelarXML";

            var URL_WS = "";
            URL_WS = jsonEnableFeat.host + "/ERPWebServices/soap/CFDI33";
            ...
            var stringInput =
                ...
                '<soapenv:Body> ' +
                '<fac:cancelar> ' +
                '<usuario>' + jsonEnableFeat.user + '</usuario> ' +
                '<password>' + jsonEnableFeat.password + '</password> ' +
            ...
            try {
                ...
                } else if (jsonEnableFeat.sendType == '2') {
                    URL_WS = "https://" + URL_WS;
                    objEnvioCFE =
                        https.post({
                            url: URL_WS,
                ...
                log.debug('returnEnvioCFE', returnEnvioCFE);
         ...
         **************************************************/
        function WSConsultaCFE(jsonEnableFeat) {
            nombreWS = "WSConsultaCFE";

            var URL_WS = "";
            URL_WS = jsonEnableFeat.host + "/ws/services/CFDI?wsdl";
            ...
            var StringXML =
                ...
                '<usuario xsi:type="xsd:string">' + jsonEnableFeat.user + '</usuario> ' +
                '<password xsi:type="xsd:string">' + jsonEnableFeat.password + '</password> ' +
            ...
            if (estatus != null && estatus != '') { //SI TIENE DATA
                ...
                if (estatus == "201") {
                    ...
                    if (["204", "213", "501", "502", "503", "601",
                            ...
                            "630", "631", "632", "633", "1302", "305"
                        ].indexOf(estatus) != -1) {
                        status_doc = "No Cancelado";
                    }
                    if (estatus == "211" || estatus == "215" || estatus == "500") {
                        status_doc = "Cancelando";
                        exito = true;
                    }
                }
         ...
         **************************************************/
        function WSDescargarXML(jsonEnableFeat) {
            ...
            log.debug("returnDescargarXML:", returnDescargarXML);
         ...
         ************************************************************/
        function sendMail(jsonEnableFeat, content, WSCancelarXML, WSDescargarXML, WSConsultaCFE, PDF) {
            ...
            if (jsonEnableFeat.emailSubsi != '' && jsonEnableFeat.emailSubsi != null) {
                var tempEmail = [emailUser, jsonEnableFeat.emailSubsi];
                EmailDest = tempEmail;
            } else {
                EmailDest = [emailUser];
            }
        ...
        function saveCartaPorte(internalIdCarta, numberCarta) {
            ...
            log.debug('se guardo el record', 'Se guardo el record');
        }
        ...
        function getOtherFields(_nTypeRec, _nSerieDoc, _nTypeDoc) {
            ...
            log.debug("_nSerieDoc - _nTypeDoc", _nSerieDoc + "-" + _nTypeDoc);
            ...
            if ((_nSerieDoc != '' && _nSerieDoc != null) && (_nTypeDoc != '' && _nTypeDoc != null)) {
                ...
                log.debug("Folio Fiscal Ref- cod", folioFiscalRef + '-' + codModificacion);
                serie = ser.name;
                ...
                var columns_srch = [
                    ...
                    'custbody_lmry_serie_doc_cxc',
                    'custbody_lmry_document_type',
                    'custbody_lmry_modification_reason.custrecord_lmry_cod_modification_reason',
                    'custbody_lmry_foliofiscal_doc_ref'
                ];
                if (!subsi) {
            ...
            log.debug("nameTypeDoc - _IniTypeDoc", nameTypeDoc + ' - ' + _IniTypeDoc);
            ...
            log.debug('Valores 1:', subsidiary + '-' + serie + '-' + folio + '-' + desing + '-' + initialCountry + '-' + subsiTransactext);
        }
        ...
        function Remove_Trans(_recordId, recordType, void_feature, jsonEnableFeat) {
        ...
        function isCurrentMonth(p_nInternalid, p_nTypeDoc) {
            var record_month = search.lookupFields({
                type: p_nTypeDoc,
                id: p_nInternalid,
                columns: 'trandate'
            }).trandate
            record_month = format.parse({
                value: record_month,
