...
define(["N/query", "N/search", "N/record", "N/log","N/runtime","N/format"], function (query, search, record, log, runtime, format) {
    ...
    function createMXTransactionbyPediment(recordObj, isReceiptDeferral, idPurchaseOrder) {
        try {
            ...
            log.error("error [createMXTransactionbyPediment]",error)
        }
    ...
    function isValidItemsTransaction(recordObj) {
        ...
        items.forEach(function (itemLine) {
            ...
            if (quantitytotal > sumQuantityDisp) {
                message = '⚠️ NO SE CREARÁN PEDIMENTOS \n No existe stock suficiente con pedimentos registrados para este artículo y ubicación.\nVerifique los registros de pedimentos o realice un ajuste de inventario antes de continuar.';
            } else {
                for (var i = 0; i < listPediment.length; i++) {
                ...
                if (quantitytotal > 0) message = '⚠️ NO SE CREARÁN PEDIMENTOS \n No existe stock suficiente con pedimentos registrados para este artículo y ubicación.\nVerifique los registros de pedimentos o realice un ajuste de inventario antes de continuar.';
            };
        });
        return message;
    }
    function getItems(recordObj) {
        ...
        if (recordShipment.type == "inventorytransfer") {
            ...
            for (var i = 0; i < numItems; i++) {
                ...
                var checkPediment = recordShipment.getSublistValue({ sublistId: 'inventory', fieldId: 'custcol_lmry_mx_pediment', line: i });
    ...
    function updateLinesUsePedimentos(recordObj,isClient) {
        var sublistId = "item";
        var transactionType = recordObj.type;
        if (transactionType == "inventorytransfer" || transactionType == "inventoryadjustment") sublistId = "inventory";
        ...
        if (listItems.length) {
            ...
            for (var index = 0; index < nLines; index++) {
                ...
                if (Object.keys(listUsePedimentos).indexOf(itemID) != -1) {
                    if (isClient) {
                        line.setCurrentSublistValue({
                            sublistId: sublistId,
                            fieldId: 'custcol_lmry_mx_pediment',
                            value: listUsePedimentos[itemID]
                        });
