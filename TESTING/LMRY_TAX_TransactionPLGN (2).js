        ...
        if (!isTaxCalculator) {
            ...
            if (salesTransactionTypes.indexOf(transactionType) != -1) {
                ...
                if (getAuthorization(527, licenses) == true && (transactionRecord.id != null && transactionRecord.id != '')) {
                    ...
                    nlapiLogExecution("ERROR", "results ", JSON.stringify(results));
                    if (results && results.length) {
                        for (var i = 0; i < results.length; i++) {
                            ...
                            if (parseFloat(amount) <= 0) {
                                nlapiLogExecution("ERROR", "amount stop", parseFloat(amount));
                                continue;
                            }
                            ...
                            if (Number(apTaxFlow) == 4) {
                                if ((isTaxSubstitution == "T" || isTaxSubstitution == true) || (isTaxNotIncluded == "T" || isTaxNotIncluded == true)) {
                                    nlapiLogExecution("ERROR", "isTaxSubstitution stop", isTaxSubstitution);
                                    nlapiLogExecution("ERROR", "isTaxNotIncluded stop", isTaxNotIncluded);
                                    continue;
                                }
                            }
                            ...
                            if (ccl != null && ccl != '') {
                                ...
                                if ((isImportTax == "T" || isImportTax == true) && Number(importTaxLevel) != 1) {
                                    nlapiLogExecution("ERROR", "isImportTax stop", isImportTax);
                                    nlapiLogExecution("ERROR", "importTaxLevel stop", importTaxLevel);
                                    continue;
                                }
                                ...
                                nlapiLogExecution("ERROR", "continue standar stop", "standar");
                                continue;
                            }
