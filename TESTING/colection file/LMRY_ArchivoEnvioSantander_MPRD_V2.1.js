...
define([
    ...
    let codigoBanco = '';
    let CodigoTransmision = '';  // Enhancement C1273
    ...
    function getInputData() {
        try {
            ...
            // C1282 : Different Nosso Número
            // Latam - BR Our Number Generated   : custrecord_lmry_br_nosso_nro_generated
            // Latam - BR Our Number Installment : custrecord_lmry_br_nosso_nro_installment
            ...
            const columns = [
                ...
                'custrecord_lmry_br_related_transaction.custrecord_lmry_br_nosso_nro_generated',
                'custrecord_lmry_br_related_transaction.custrecord_lmry_br_nosso_nro_installment'
            ];
            ...
            // C1282 : Different Nosso Número
            FeaInstallment = libraryMail.getAuthorization(536, licenses);

            if ((FEATURE_INST == 'T' || FEATURE_INST == true) && FeaInstallment) {
                ...
                columns.push('installment.fxamountremaining');
    ...
    function map(context) {
        try {
            ...
            AMOUNT_INST    = searchResult.values['fxamountremaining.installment'];
            DUEDATE_INST   = searchResult.values['duedate.installment'];
            // C1282 : Different Nosso Número
            var nosogene = searchResult.values['custrecord_lmry_br_nosso_nro_generated.custrecord_lmry_br_related_transaction'];
            if ((nosogene=='T' || nosogene==true) && NOSSO_NUMBER==''){
                try{
                    // Latam - BR Our Number Installment
                    var nosonuro =  searchResult.values['custrecord_lmry_br_nosso_nro_installment.custrecord_lmry_br_related_transaction'];
                    nosonuro = JSON.parse(nosonuro);
                    if (ID_INST){
                        if (nosonuro[ID_INST] && Object.keys(nosonuro[ID_INST]).length > 0){
                            if (nosonuro[ID_INST]!='' && nosonuro[ID_INST]!=null){
                                NOSSO_NUMBER = nosonuro[ID_INST]["nossonumero"];
                            }
                        }
                    }else{
                        NOSSO_NUMBER = '';
                    ...
                    NOSSO_NUMBER = '';
                    log.error('map : errmsg', errmsg.stack);
                    log.error('map : errmsg', errmsg);
                    log.error('map : Latam - BR Our Number Installment', nosonuro);
                    log.error('map : ID_INST', ID_INST);
                    log.error('map : AMOUNT_INST : ' + AMOUNT_INST, 'DUEDATE_INST : ', DUEDATE_INST);  
                    log.error('map : nosonuro[ID_INST]', nosonuro[ID_INST]);
                    //log.error('map : Object.keys(nosonuro[ID_INST].nossonumero).length', Object.keys(nosonuro[ID_INST].nossonumero).length);
                }
            }
            // Datos Transaccion Fields
            const documentEspecie = searchResult.values['custrecord_lmry_br_document_specie.custrecord_lmry_br_related_transaction'].value;
            const status          = searchResult.values['custrecord_lmry_br_bb_status.custrecord_lmry_br_related_transaction'].value;
            const processingDate  = searchResult.values['custrecord_lmry_br_processing_date.custrecord_lmry_br_related_transaction'];
        ...
        try {
            ...
            const searchConfigSubsiBank = search.create({
                ...
                columns: [
                    ...
                    {
                        name: 'custrecord_lmry_code_transmission'
                    }
                ],
                filters: [
                    ['isinactive', 'is', 'F'],
            ...
            if (searchResult != '' && searchResult != null) {
                ...
                // Codigo de Transmision - Enhancement C1273
                CodigoTransmision = searchResult[0].getValue(columns[7]);
    ...
    function obtenerCabeceraArchivo() {
        ...
        string += completarFinal(CodigoTransmision, 20, '0'); // Código de Transmissão N(20) - Enhancement C1273
    ...
    function completarInicio(dato, cantMaxima, caracter) {
		try{
			...
			log.error('completarInicio : error', error);
			dato = '';
		}
    ...
    function completarFinal(dato, cantMaxima, caracter) {
		try{
			...
			log.error('completarFinal : error', error);
			dato = '';
		}
        if (dato.lentgh == cantMaxima) {
            return dato;
