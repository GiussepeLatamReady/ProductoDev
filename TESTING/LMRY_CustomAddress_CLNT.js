...
define(["N/search", "N/currentRecord", "N/runtime"], function (search, currentRecord, runtime) {
    function brCustomCity() {
        ...
        <span id="custrecord_lmry_addr_city_fs_lbl_uir_label" class="smallgraytextnolink uir-label" data-nsps-type="field_label"
            ><span id="custrecord_lmry_addr_city_fs_lbl01" class="labelSpanEdit smallgraytextnolink" data-nsps-type="label">
                <a
                    ...
                    >${recordNow.getField("custrecord_lmry_addr_city").label}</a
                >
            </span>
        </span>
        <span class="uir-field" data-nsps-type="field_input" >
            ...
            if(country && country == 'CO'){
                ...
                const FieldCustom = searchValidationFields();

                // Llama la función de validación
                if (document.getElementById('submitter')) {
                    validarCamposDireccion('#submitter', FieldCustom);
                }
                if (document.getElementById('ok')) {
                    validarCamposDireccion('#ok', FieldCustom);
                }
            }
    }

    // Consultar los campos de validación
    function searchValidationFields() {
        const searchFields = search.create({
            type: 'customrecord_lmry_validation_fields',
            filters: [
                ['custrecord_lmry_val_section', 'is', '8'],
                'AND',
                ['custrecord_lmry_validation_country', 'is', 'Colombia'],
                'AND',
                ['custrecord_lmry_mandatory', 'is', 'T']
            ],
            columns: [
                search.createColumn({ name: 'name' })
            ]
        });
        const resultados = searchFields.run().getRange({ start: 0, end: 100 });
        if (!resultados || resultados.length === 0) {
            return [];
        }
        const parsedFields = resultados.map(function (result) {
            const inputId = result.getValue({ name: 'name' }); // ID del input
            const labelSpanId = inputId + '_fs_lbl';           // ID del label

            // Obtener el texto del <a> dentro del span del label
            let labelText = '';
            const labelSpan = document.getElementById(labelSpanId);
            if (labelSpan) {
                const link = labelSpan.querySelector('a');
                if (link) {
    ...
    function validarCamposDireccion(btnSelector, fields) {
        const btn = document.querySelector(btnSelector);
        if (!btn) return;

        // Pintar de amarillo los label
        fields.forEach(function (fieldObj) {
            let labelId = fieldObj.label;
            if (labelId === 'custrecord_lmry_addr_city_fs_lbl') {
                labelId = 'custrecord_lmry_addr_city_fs_lbl01';
            }
        ...
        // Validar campos al hacer clic
        btn.addEventListener("click", function (event) {
            let camposVacios = [];

            fields.forEach(function (fieldObj) {
                let fieldId = fieldObj.input;
                const labelId = fieldObj.label;
                const mensaje = fieldObj.message || labelId;

                let campo = null;

                if (fieldId === 'custrecord_lmry_addr_city') {
                    fieldId = 'custpage_br_city';
                }

                campo = document.querySelector(`#${fieldId}`);

                if (!campo) {
                    const testName = `inpt_${fieldId}`;
                    const posiblesCampos = document.getElementsByName(testName);
                    if (posiblesCampos && posiblesCampos.length > 0) {
                        campo = posiblesCampos[0];
                    }
                }

                // Validar que el campo existe y está visible
                const esCampoVisible = campo &&
                    campo.offsetParent !== null &&
                    campo.type !== 'hidden'; 

                if (esCampoVisible && !campo.value.trim()) {
