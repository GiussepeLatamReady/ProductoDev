/* = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =\
||   This script for Boleto Bancario (Brazil)                   ||
||                                                              ||
...
define(['N/record', 'N/file', 'N/record', 'N/render', 'N/url', 'N/log', 'N/xml', 'N/https', 'N/search', 'N/runtime', 'N/format', 'N/format/i18n', 'N/config', './LMRY_libSendingEmailsLBRY_V2.0'],
  function (record, file, record, render, url, log, xml, https, search, runtime, format, format2, config, libraryMail) {
    ...
    var INSTRUCTIONS = '';
    var TYPE_COLLECTION = '';
    // C1272 : Solo enviar Installment
    var ONLY_INSTALLMEN = false;
    var JSON_INSTALLMEN = {};
    ...
    function obtenerURLPDFBoletoBancario(id_invoice, id_recordConfigBankTicket, statusCodOcurencia, installment, type) {
      try {
        ...
        // C1282 : Different Nosso Número 
        var instalquanty = 0;

        if (ONLY_INSTALLMEN == true || ONLY_INSTALLMEN == "T") {
          instalquanty = createJSonInsallment(id_invoice);
        }
        // Solo si no hay installment se ejecuta
        ...
        BOLETOS = formarInstallments(installment, instalquanty);

        return BOLETOS;

      } catch (err) {
        log.error('ERROR [obtenerURLPdfBoletoBancario]', err);
        libraryMail.sendemail(' [ obtenerURLPdfBoletoBancario ] ' + err, LMRY_script);
      }
    }

    function formarInstallments(installment, instalquanty) {
      try {
        var linkBoleto = '';
        var arrayBoletos = [];
        ...
        if (FOLDER_BR) {
          ...
          /* **************************************************
           * C1272 : Si el Invoice tiene Installment
           * => ONLY_INSTALLMEN = true
           *    - No crea la factura por el total
           * => installment = vacio
           *     - crea la factura por el total
           * ************************************************ */
          if (installments == false || installments == 'F') {
            // Crea PDF de Boleto Bancario
          ...
          if (installments && libraryMail.getAuthorization(536, licenses)) {
            ...
            if (Object.keys(jsonInstallments).length) {
              ...
              for (var idInstallment in jsonInstallments) {
                ...
                // 2025.05.03 : nossonumero del JSon
                if (installment != '' && installment != null && (ONLY_INSTALLMEN == true || ONLY_INSTALLMEN == "T")) {
                  JSON_INSTALLMEN = obtenerJSON(JSON_INSTALLMEN);
                  NOSSO_NUMBER = JSON_INSTALLMEN[posicion].nossonumero;
                }
                // Crea PDF de Boleto Bancario
              ...
              if (resultInstallments != null && resultInstallments != '' && installNumber != '' && installNumber != null) {
                ...
                for (var i = 0; i < resultInstallments.length; i++) {
                  ...
                  var ins_fxamou = resuQuote[posResu].getValue(columns[3]);
                  monto = parseFloat(monto);
                  ins_fxamou = parseFloat(ins_fxamou);
                  ...
                  /* *********************************
                   * C1282 : Different Nosso Número
                   * Se obtiene el Nosso Numero de 
                   * JSON Insallment
                   ********************************* */
                  if (instalquanty > 1 && (ONLY_INSTALLMEN == true || ONLY_INSTALLMEN == 'T')) {
                    NOSSO_NUMBER = JSON_INSTALLMEN[i].nossonumero;
                  }
          ...
          // Actualiza la transaccion con los installments
          if (TYPE == '1') {
            record.submitFields({
              type: 'invoice',
              id: ID_INV,
        ...
        log.error(' [ formarInstallments ] ', err)
      ...
      try {
        ...
        ***************************************************************************************************************/
        ...
        if (libraryMail.getAuthorization(416, licenses) == true && tax_code_des == 'Latam - WHT Tax') {
          FLAG = true;
          var retencion = 0;
          if (recordInvoice.custbody_lmry_wtax_amount) {
            retencion = recordInvoice.custbody_lmry_wtax_amount;
          }
    ...
    function obtenerDatosSubsidiaria() {
      try {
        ...
        log.error("ERROR [obtenerDatosSubsidiaria]",err)
    ...
    function obtenerDatosSacadorAvalista() {
      try {
        ...
        log.error("ERROR [obtenerDatosSacadorAvalista]",err)
      ...
      try {
        ...
        var searchConfigSubsiBank = search.create({
          ...
          columns: [{
            ...
            name: 'custrecord_lmry_nossonro_onlyinstallment'
          }],
        ...
        if (searchResult != '' && searchResult != null) {
          ...
          // Custom Field => Our Correlative Number
          // Field ID     => custrecord_lmry_nossonumero_correlative
          ...
          /* *********************************
           * C1272 : Solo enviar Installment
           * Latam - BR Only Installment	
           * custrecord_lmry_nossonro_onlyinstallment
           * ****************************** */
          ONLY_INSTALLMEN = searchResult[0].getValue(columns[13]);
        ...
        log.error('obtenerDatosConfigBankTicket : ', err);
      ...
      try {
        ...
        if (searchResult != '' && searchResult != null && searchResult.length > 0) {
          ...
          if (brNossoNumero == '' || brNossoNumero == null) {
            ...
            // 2025.03.19 : se agrego el parametro options
            ...
            var id = record.submitFields({
              ...
              values: {
                'custrecord_lmry_br_nosso_nro_generated': true,
    ...
    function calcularDVNossoNumero(nosso_number) {
      try {
        ...
        log.error("ERROR [ calcularDVNossoNumero ]",err)
    ...
    function formarInstrucciones() {
      try {
        ...
        log.error("ERROR [ formarInstrucciones ]",err)
    ...
    function obtenerEspecieDOC() {
      try {
        ...
        log.error("ERROR [obtenerEspecieDOC]",err)
        libraryMail.sendemail(' [ reemplazarCaracteresEspeciales ] ' + err, LMRY_script);
      }
    }
    ...
    function reciboPagador(monto) {
      try {
        ...
        html += "</tr>";
        html += "<tr>";
        html += "<td style=\"font-size: 8pt; border-left: 1px; border-bottom:1px; min-height:16px\" align=\"left\" colspan=\"1\" >";
        html += NOSSO_NUMBER;
        html += "</td>";
        html += "</tr>";
        html += "<tr >";
    ...
    function fichaCompensacion(monto) {
      try {
        ...
        html += "<td style=\"font-size: 8pt; border-left:1px\" align=\"right\" colspan=\"2\">";
        html += NOSSO_NUMBER
        html += "</td>";
        html += "</tr>";
    ...
    function search_folder() {
      try {
        ...
        log.error("ERROR [search_folder]",err)
    ...
    /* *********************************
      ...
      * Se agrego el Disabke Triggers
      * ****************************** */
    function createJSonInsallment(id_invoice) {
      var instalquanty = 0;
      try {
        ...
        if (FEAT_INSTALLMENTS == "T" || FEAT_INSTALLMENTS == true) {
          ...
          var resuQuote = searchInstallments.run().getRange(0, 1000);

          // Si no hay cuotas no hace nada
          instalquanty = resuQuote.length;
          if (instalquanty < 2) {
            return instalquanty;
          }
          ...
          var searchBRTransactionFields = search.create({
            ...
            filters: [
              ["isinactive", "is", "F"], "AND",
              ["custrecord_lmry_br_related_transaction", "anyof", id_invoice]
            ],
            columns: [{
              name: 'internalid'
            }, {
              ...
              name: 'custrecord_lmry_br_nosso_nro_generated'
            }, {
              name: 'custrecord_lmry_br_nosso_nro_installment'
            }],
          });

          var resultsBRTF = searchBRTransactionFields.run().getRange(0, 10);
          if (resultsBRTF && resultsBRTF.length) {
            // Para el PDF del Boleto Bancario
            ID_SACADOR_AVALISTA = resultsBRTF[0].getValue('custrecord_lmry_br_sacador_avalista');

            // Validacion si se genero el Nosso Numero
            var idBrTransactionField = resultsBRTF[0].getValue("internalid");
            var idBrNossoNroGenerate = resultsBRTF[0].getValue("custrecord_lmry_br_nosso_nro_generated");

            // Actualiza el registro personalizado
            if (idBrNossoNroGenerate == false || idBrNossoNroGenerate == 'F') {
              //Se forma Nosso Numero (Correlativo de 11 dígitos + Dígito verificador)
              var maxLimit = 7;
              var nossoNumCorrelativo = 0;

              /* ********************************
              * Separamos los Nosso Numeros
              * ****************************** */
              nossoNumCorrelativo = parseInt(CORRELATIVE_NOSSO_NUMBER) + parseInt(instalquanty);
              ...
              // Asigna Nosso Numero
              for (var posResu = 0; posResu < resuQuote.length; posResu++) {
                // Ceros para Nosso Numero
                var llenarCeros = '';
                var nossonumber = '';

                // Datos del Installment
                var ins_number = resuQuote[posResu].getValue(columns[0]);
                var ins_amount = resuQuote[posResu].getValue(columns[1]) || 0.00;
                ins_amount = parseFloat(ins_amount);
                var ins_duedat = resuQuote[posResu].getValue(columns[2]);
                var ins_intern = resuQuote[posResu].getValue(columns[3]);
                var ins_fxamou = resuQuote[posResu].getValue(columns[4]);

                // Incremente el correlativo
                CORRELATIVE_NOSSO_NUMBER = parseInt(CORRELATIVE_NOSSO_NUMBER) + 1;

                // Genera Nosso Numero para Installment
                nossoNumCorrelativo = parseInt(CORRELATIVE_NOSSO_NUMBER);
                var longNumeroConsec = parseInt((nossoNumCorrelativo + '').length);
                // Lenna de ceros
                for (var qtyZero = 0; qtyZero < (maxLimit - longNumeroConsec); qtyZero++) {
                  llenarCeros += '0';
                }

                nossonumber = llenarCeros + nossoNumCorrelativo;

                // Ejecuta calcular DV Nosso Numero
                var dvNossoNumero = calcularDVNossoNumero(nossonumber);

                nossonumber = llenarCeros + nossoNumCorrelativo + dvNossoNumero;

                // Crea JSon
                if (ins_number) {
                  installments[String(ins_number)] = {
                    "internalid": ins_intern + '-' + String(ins_number),
                    "amount": ins_amount,
                    "duedate": ins_duedat,
                    "nossonumero": nossonumber,
                    "fxamount": ins_fxamou
                  };
                }
              }
              ...
              JSON_INSTALLMEN = installments;

              // LatamReady - BR Transaction Fields (Nosso Numero)
              record.submitFields({
                type: "customrecord_lmry_br_transaction_fields",
                id: idBrTransactionField,
                values: {
                  "custrecord_lmry_br_nosso_nro_generated": true,
                  "custrecord_lmry_br_nosso_nro_installment": JSON.stringify(installments)
                },
                options: {
                  ignoreMandatoryFields: true,
                  enableSourcing: true,
              ...
              JSON_INSTALLMEN = obtenerJSON(resultsBRTF[0].getValue("custrecord_lmry_br_nosso_nro_installment"));
            } // Actualiza el registro personalizado
          }
        } // FEAT_INSTALLMENTS
      } catch (err) {
        log.error('createJSonInsallment : ', err);
        libraryMail.sendemail(' [ createJSonInsallment ] ' + err, LMRY_script);
      }
      return instalquanty;
    }

    // Validacion de Cadena
    function obtenerJSON(cadena) {
      try {
        // Intentar parsear la cadena
        return JSON.parse(cadena);
      } catch (e) {
        // Si ocurre un error, probablemente ya es un objeto
        if (typeof cadena === 'object') {
          return cadena; // Ya estaba parseado
        } else {
          // Error genuino, no es un JSON válido
          log.error('obtenerJSON : La cadena no es un JSON válido:', e);
          return '';
        }
      }
    }
