    ...
    function afterSubmitTransaction(scriptContext, isUret, typeClnt) {
      ...
      try {
        ...
        if (type == 'create' || type == 'edit' || type == 'copy') {
          ...
          var resultCC = calculateTaxesByItem(recordObj, ccs, setupTax, mvaRecords, discountAmounts, exchangeRate, customSegments);
          taxResults = taxResults.concat(resultCC);

          if (!taxResults.length) {
            var nts = getNationalTaxes(recordObj, setupTax);
            log.debug("nts", JSON.stringify(nts));
            var resultNT = calculateTaxesByItem(recordObj, nts, setupTax, mvaRecords, discountAmounts, exchangeRate, customSegments);
            taxResults = taxResults.concat(resultNT);
          }
      ...
      try {
        ...
        if (idCountry == 30 && transactionType == 'invoice') {
          ...
          var itemOriginSearch = search.create({
            ...
            columns: ["internalid", "custitem_lmry_br_origem_do_item"]
          });
          ...
          if (resultsItemOrigin && resultsItemOrigin.length) {
            for (var i = 0; i < resultsItemOrigin.length; i++) {
              var itemOrigin = resultsItemOrigin[i].getValue("custitem_lmry_br_origem_do_item") || "";
          ...
          if (catalogs.length) {
            ...
            if (Object.keys(catalogLawItem).length) {
              ...
              var porcFedNacGlobal = 0;
              var porcFedImpGlobal = 0;
              var porcEstGlobal = 0;
              var porcMunGlobal = 0;
                  ...
                  for (var i = 0; i < resultTransFields.length; i++) {
                    ...
                    record.submitFields({
                      ...
                      values: {
                        ...
                        custrecord_lmry_br_trib_fed_nac_p: porcFedNacGlobal,
                        custrecord_lmry_br_trib_fed_imp: sumFedImp,
                        custrecord_lmry_br_trib_fed_imp_p: porcFedImpGlobal,
                        custrecord_lmry_br_trib_est: sumEst,
                        custrecord_lmry_br_trib_est_p: porcEstGlobal,
                        custrecord_lmry_br_trib_mun: sumMun,
                        custrecord_lmry_br_trib_mun_p: porcMunGlobal,
                      },
                      options: {
