  ...
  function (library_Uni_Setting, library_hideview3, Library_BoletoBancario, Library_Mail, Library_Number,
    ...
    function beforeLoad(scriptContext) {
      try {
        ...
        var FEAT_LANG = runtime.getCurrentScript().getParameter({
          name: 'LANGUAGE'
        });
        FEAT_LANG = FEAT_LANG.substring(0, 2);
        ...
        if (type_interface == 'USERINTERFACE' && (scriptContext.type == 'create' || scriptContext.type == 'copy' || scriptContext.type == 'edit')) {
          library_Uni_Setting.createTemplate(scriptContext, LMRY_Result[0]);
          library_Uni_Setting.setAutomatiSetTemplate(scriptContext, LMRY_Result[0], true);
        }
        ...
        if (scriptContext.type == 'create' || scriptContext.type == 'copy' || scriptContext.type == 'edit') {
          if (LMRY_Result[0] === "BR" && type_interface == 'USERINTERFACE'){
        ...
        if (scriptContext.type != 'print' && scriptContext.type != 'email') {
          ...
          if (LMRY_Result[0] == 'CO') {
            ...
            if ((Library_Mail.getAuthorization(340, licenses) || Library_Mail.getAuthorization(721, licenses)) && !isSuiteAppInstalledCO) {
            ...
            if (!Library_Mail.getAuthorization(721, licenses) && !isSuiteAppInstalledCO) {
              var formObj = scriptContext.form;
            ...
            //CO - Pre-Printer Number
            var validateCoSerie = libTools.getSetupCoSerieNacional(subsidiari);
            if((scriptContext.type == 'create' || scriptContext.type == 'copy' || scriptContext.type == 'edit') && type_interface == 'USERINTERFACE' &&  validateCoSerie){
              var pageSerieCxC = OBJ_FORM.addField({
                id: 'custpage_lmry_serie_doc_cxc',
                type: serverWidget.FieldType.TEXT,
                label: 'Latam - Serie CXC'
              });
              OBJ_FORM.insertField({
                  field: pageSerieCxC,
                  nextfield: 'custbody_lmry_serie_doc_cxc'
              }); 
            }
          }
        }
        ...
        if (LMRY_Result[0] == 'CO' && Library_Mail.getAuthorization(721, licenses) && !isSuiteAppInstalledCO) {
        ...
        if (LMRY_Result[0] == 'BR' && (scriptContext.type == "create" || scriptContext.type == "edit" || scriptContext.type == "view" || scriptContext.type == "copy") && type_interface == 'USERINTERFACE') {
        ...
        if (scriptContext.type == 'create' && iscreateAS && (["USERINTERFACE", "USEREVENT", "CSVIMPORT", "RESTWEBSERVICES", "RESTLET", "WEBSERVICES"].indexOf(type_interface) != -1) && (ST_FEATURE == false || ST_FEATURE == "F")) {
          library_Uni_Setting.runAutomaticSet(RCD_OBJ, licenses, LMRY_Result);
        }
    ...
    function beforeSubmit(scriptContext) {
      try {
        ...
        if (scriptContext.type == "create") {
          if (RCD.getValue('entity') != '' && RCD.getValue('entity') != null) {
            Library_ExchangeRate.ws_exchange_rate_uret(RCD, licenses);
          }
        }
        ...
        if (type_interface == 'USERINTERFACE' && (scriptContext.type == 'create' || scriptContext.type == 'copy' || scriptContext.type == 'edit')) {
          library_Uni_Setting.setAutomatiSetTemplate(scriptContext, LMRY_Result[0], false);
        }
        ...
        if (type_event == 'create' && (["USERINTERFACE", "USEREVENT", "CSVIMPORT", "RESTWEBSERVICES", "RESTLET", "WEBSERVICES"].indexOf(type_interface) != -1) && (ST_FEATURE == false || ST_FEATURE == "F")) {
          library_Uni_Setting.runAutomaticSet(RCD, licenses, LMRY_Result);
        }
        if (type_event == 'edit') {
        ...
        //Seteo Latam - Serie CxC
        var validateCoSerie = libTools.getSetupCoSerieNacional(subsidiary);
        if (LMRY_Result[0] == 'CO' && LMRY_Result[2] === true && validateCoSerie) {
          var docType = RCD.getValue({ fieldId: 'custbody_lmry_document_type' });
          var serieDoc = null;
          if (docType) {
              var documents = libTools.searchNumberPrinted(subsidiary);
              if (documents) {
                  if (docType == documents.document1) {
                      serieDoc = documents.serieDoc1;
                  } else if (docType == documents.document2) {
                      serieDoc = documents.serieDoc2;
                  }
              }
              if (serieDoc) {
                  RCD.setValue({
      ...
      try {
        ...
        if (LMRY_Result[0] == 'BR') {
          ...
          if (["create", "edit", "copy"].indexOf(scriptContext.type) != -1) {
            ...
            //boleto bancario
            Library_BoletoBancario.createPdfbankCheque(LMRY_Intern, 1, 1);
          }
        }

        var validateCoSerie = libTools.getSetupCoSerieNacional(subsidiary);
        if (LMRY_Result[0] == 'CO' && LMRY_Result[2] == true && validateCoSerie && (type_event === 'create' || type_event === 'copy')) {
          var docType = RCD.getValue({ fieldId: 'custbody_lmry_document_type' });
          if (docType) {
              var documents = libTools.searchNumberPrinted(subsidiary);
              if (documents) {
                  // Validar docType
                  if (docType == documents.document1 || docType == documents.document2) {
                      var ultimoNumero = parseInt(documents.serie_num, 10) || 0;
                      var nuevoNumero = ultimoNumero + 1;
                      // Actualizar registro
                      record.submitFields({
                          type: "customrecord_lmry_co_number_preprinted",
                          id: documents.id,
                          values: {
                              custrecord_lmry_co_np_serie_numero: nuevoNumero
                          },
                          options: {
    ...
    function afterSubmit_newedi(RCD, scriptContext, LMRY_Result, ORCD) {
      ...
      try {
        if (RCD.getValue({
          ...
          var swAccess = false;
          if (LMRY_Result[0] == 'CO') {
            swAccess = Library_Mail.getAuthorization(27, licenses) || Library_Mail.getAuthorization(1130, licenses);
          }
          if (LMRY_Result[0] == 'BO') {
    ...
    function createCOLineFieldsWHT(formObj, recordObj, type) {
      try {
        ...
        var filters = [
          ...
          "AND",
          ["custrecord_lmry_ntax_transactiontypes", "anyof", "1"],
          "AND",
