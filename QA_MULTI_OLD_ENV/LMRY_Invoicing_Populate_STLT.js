                                        ...
                                        if (hasEIMXLMRYProd == true || hasEIMXLMRYProd == 'T' || hasEIMXLMRYTest == true || hasEIMXLMRYTest == 'T') {
                                            if (existCustomfield) {
                                                ...
                                                if (Rd_Country == 'MEX' && id_type == "custpymt") {
                                                    var contadorPUE = 0;
                                                    var applySearch = search.create({
                                                        type: "invoice",
                                                        filters: [
                                                           ["applyingtransaction.internalidnumber", "equalto", Number(id_invoice)]
                                                        ],
                                                        columns: [
                                                           search.createColumn({
                                                              name: "custrecord_lmry_mx_code_payment_method",
                                                              join: "CUSTBODY_LMRY_MX_PAYMENTMETHOD",
                                                              label: "Latam - MX Code Payment Method"
                                                           })
                                                        ]
                                                    });
                                                    var myPagedData = applySearch.runPaged({
                                                        pageSize: 1000
                                                    });
                                                    myPagedData.pageRanges.forEach(function (pageRange) {
                                                        ...
                                                        myPage.data.forEach(function (result) {
                                                            ...
                                                            var code = result.getValue(columns[0]);
                                                            code = code.toUpperCase();
                                                            if (code == 'PUE') contadorPUE++;
                                                        });
                                                    });
                                                    if (contadorPUE > 0) continue;
                                                }
                                            }
                                        }
