  ...
  function(search, https, log, email, runtime, record, url, format, serverWidget) {
    ...
    function sendemail(err, LMRY_script) {
      try {
        ...
        log.debug('sendemail - userid', userid);
        ...
        log.debug('sendemail - empema - empfir', empema + ' - ' + empfir);
        ...
        log.debug('type , message :' + posExist1 + ' , ' + posExist2, 'ReferenceError : ' + posExist3)
        ...
        log.debug(' [ sendemail : UserID ] ' + userid, LMRY_script);
        email.send({
          author: userid,
          recipients: empema,
          subject: subject,
