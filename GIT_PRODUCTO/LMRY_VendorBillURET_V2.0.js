...
define(['require', './Latam_Library/LMRY_UniversalSetting_Purchase_LBRY', 'N/record', 'N/runtime', 'N/search', 'N/log', 'N/config', 'N/https', 'N/url', 'N/ui/serverWidget',
    ...
    './Latam_Library/LMRY_PE_STE_Purchases_Tax_Transaction_LBRY_V2.0'
],
    ...
    function (require, library_Uni_Setting, record, runtime, search, log, config, https, url, serverWidget, library, library1, libraryDIOT, Library_WHT_Transaction, ST_Library_Transaction,
        ...
        MX_STE_WhtLibrary_Total, libraryMxJsonResult, PE_STE_TaxLibrary) {
        ...
        var recordObj = null;
        var licenses = [];
        var switchThrow = false, msgThrow = { msgName: 'ERROR_NAME', msgError: { en: 'ERROR_MSG' } };
            ...
            try {
                ...
                if ((type == 'create' || type == 'edit' || type == 'copy' || type == 'view') && LMRY_Result[0] == 'UY') {
                    ...
                    var TaxRuleSearchObj = search.create({
                        ...
                        filters: [["custrecord_lmry_taxrule_country", "anyof", "231"]],
                ...
                if ((type == 'create' || type == 'edit' || type == 'copy' || type == 'view') && LMRY_Result[0] == 'MX' && featWhtMx) {
                    ...
                    var TaxRuleSearchObj = search.create({
                        ...
                        filters: [["custrecord_lmry_whtrule_country", "anyof", "157"]],
                        columns: ['internalid', 'name']
                    });
                    TaxRuleSearchObj = TaxRuleSearchObj.run().getRange(0, 1000);
                    if (TaxRuleSearchObj != null && TaxRuleSearchObj != '') {
        ...
        function beforeSubmit(context) {
            ...
            try {
                ...
                */
                var type_interface = runtime.executionContext;
                if ((type == 'create' || type == 'edit' || type == 'copy') && type_interface != 'USERINTERFACE') {
                    ...
                    if (type_interface == 'CSVIMPORT') {
                        if (LMRY_Result[0] == 'AR' ||
                            (featureTranid[LMRY_Result[0]] != undefined &&
                                library.getAuthorization(featureTranid[LMRY_Result[0]]['exclude'], licenses))) {
                            var isGeneratedTranID = libraryTranIdCSV.generateTranID(recordObj, LMRY_Result[0], licenses);
                            log.debug('isGeneratedTranID', isGeneratedTranID);
                ...
                if (ST_FEATURE == false || ST_FEATURE == "F") {
                    ...
                    if ((type == 'create' || type == 'edit' || type == 'copy') && LMRY_Result[0] == 'MX' && featWhtMx && (applWTH == 'T' || applWTH == true)) {
                        ...
                        if (cantidad_items > 0 && taxRule != '') {
                            ...
                            var cantidad_exp = recordObj.getLineCount({ sublistId: 'expense' });
        ...
        function setWithholdingValues(country, recordObj, licenses) {
            ...
            if (swFeature == true && ['USEREVENT', 'SCHEDULED'].indexOf(runtime.executionContext) == -1) {
                ...
                for (var i = 0; i < numItems; i++) {
                    var appliesLine = recordObj.getSublistValue({ sublistId: "item", line: i, fieldId: "custcol_4601_witaxapplies" }) || false;
                    var taxLine = recordObj.getSublistValue({ sublistId: "item", line: i, fieldId: "custcol_4601_witaxline" }) || false;
                    if ((appliesLine == true || appliesLine == "T") && (taxLine == false || taxLine == "F")) {

                        var whtTaxCode = recordObj.getSublistValue({ sublistId: "item", line: i, fieldId: "custcol_4601_witaxcode" }) || "";
                        var whtTaxRate = recordObj.getSublistValue({ sublistId: "item", line: i, fieldId: "custcol_4601_witaxrate" }) || "";
                        ...
                        var whtBaseAmtLine = recordObj.getSublistValue({ sublistId: "item", line: i, fieldId: "custcol_4601_witaxbaseamount" }) || 0.00;
                        whtBaseAmtLine = parseFloat(whtBaseAmtLine);
                        whtBaseAmount = round2(whtBaseAmount + whtBaseAmtLine);
                        var whtTaxAmtLine = recordObj.getSublistValue({ sublistId: "item", line: i, fieldId: "custcol_4601_witaxamount" }) || 0.00;
                        whtTaxAmtLine = parseFloat(whtTaxAmtLine);
                        whtTaxAmount = round2(whtTaxAmount + whtTaxAmtLine);
                    }
                }

                var numexpenses = recordObj.getLineCount({ sublistId: "expense" });
                log.debug("numexpenses", numexpenses);
                for (var i = 0; i < numexpenses; i++) {
                    var appliesLine = recordObj.getSublistValue({ sublistId: "expense", line: i, fieldId: "custcol_4601_witaxapplies" }) || false;
                    var taxLine = recordObj.getSublistValue({ sublistId: "expense", line: i, fieldId: "custcol_4601_witaxline" }) || false;
                    if ((appliesLine == true || appliesLine == "T") && (taxLine == false || taxLine == "F")) {

                        var whtTaxCode = recordObj.getSublistValue({ sublistId: "expense", line: i, fieldId: "custcol_4601_witaxcode_exp" }) || "";
                        var whtTaxRate = recordObj.getSublistValue({ sublistId: "expense", line: i, fieldId: "custcol_4601_witaxrate_exp" }) || "";
                        ...
                        var whtBaseAmtLine = recordObj.getSublistValue({ sublistId: "expense", line: i, fieldId: "custcol_4601_witaxbamt_exp" }) || 0.00;
                        whtBaseAmtLine = parseFloat(whtBaseAmtLine);
                        whtBaseAmount = round2(whtBaseAmount + whtBaseAmtLine);
                        var whtTaxAmtLine = recordObj.getSublistValue({ sublistId: "expense", line: i, fieldId: "custcol_4601_witaxamt_exp" }) || 0.00;
                        whtTaxAmtLine = parseFloat(whtTaxAmtLine);
                        whtTaxAmount = round2(whtTaxAmount + whtTaxAmtLine);
                    }
                }
