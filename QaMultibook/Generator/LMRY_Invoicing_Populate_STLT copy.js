...
define([
        ...
        try {
            ...
            if (context.request.method == 'GET') {
                ...
                if (
                    ...
                    runtime.accountId.indexOf('TSTDRV') == 0
                ) {
                ...
                if (Rd_Country) {
                    ...
                    if (featureAdvanced) {
                        ...
                        var searchPackage = search.create({
                            ...
                            columns: [
                                ...
                                'custrecord_lmry_ei_pckg_trans_type'
                            ]
                        });
                        ...
                        if (resultPackage != null && resultPackage.length > 0) {
                            for (var i = 0; i < resultPackage.length; i++) {
                                var documento = resultPackage[i].getValue(
                                    'custrecord_lmry_ei_pckg_doc_type'
                                );
                                var transaccion = resultPackage[i].getValue(
                                    'custrecord_lmry_ei_pckg_trans_type'
                                );
                    ...
                    if (result_document != null && result_document.length > 0) {
                        for (var i = 0; i < result_document.length; i++) {
                            json_document[result_document[i].getValue('internalid')]
                                = result_document[i].getValue('internalid');
                        }
                    }
                    ...
                    var coRetencionesLines = libraryFeature.getAuthorization(
                        340,
                        licenses
                    );
                    ...
                    if (type_transaction == 'transaction') {
                        if (Rd_Country == 'BRA') {
                            ...
                            filtros_invoice[i] = search.createFilter({
                                name: 'type',
                                operator: 'anyof',
                                values: ['CustCred', 'CustInvc','VendBill', 'CustPymt']
                            });
                        ...
                        if (resultDiscountIt != null && resultDiscountIt.length) {
                            while (banderaDiscount) {
                                ...
                                for (var i = 0; i < resultDiscountIt.length; i++) {
                                    ...
                                    var idInvoiceDiscount
                                        = resultDiscountIt[i].getValue('internalid');
                    ...
                    if (result_invoice3 != null && result_invoice3.length > 0) {
                        while (bandera) {
                            ...
                            if (result_invoice2 != null && result_invoice2.length > 0) {
                                ...
                                var json_response = status_invoice(
                                    Rd_Country,
                                    transacciones,
                                    Rd_Subsi
                                );
                                ...
                                if (
                                    ...
                                    for (var i = 0; i < result_invoice2.length; i++) {
                                        transactionsId.push(
                                            result_invoice2[i].getValue('internalid')
                                        );
                                    }
                                ...
                                for (var i = 0; i < sublistLines; i++) {
                                    ...
                                    if (
                                        type_transaction
                                        == 'customtransaction_lmry_payment_complemnt'
                                    ) {
                                        id_invoice = pymntCompFields[i].getValue(
                                            'custrecord_lmry_related_complmnt_paym'
                                        );
                                        if (pymntCompDoc[id_invoice]) {
                                            id_entity = pymntCompFields[i].getText(
                                                'custrecord_lmry_complm_customer'
                                            );
                                            id_entity_value = pymntCompFields[i].getValue(
                                                'custrecord_lmry_complm_customer'
                                            );
                                            id_tran = pymntCompDoc[id_invoice].tranid;
                                            id_amount = pymntCompFields[i].getValue(
                                                'custrecord_lmry_complm_amt_dep_gross_lc'
                                            );

                                            id_doc = pymntCompFields[i].getText(
                                                'custrecord_lmry_complm_document_type'
                                            );
                                            id_doc_value = pymntCompFields[i].getValue(
                                                'custrecord_lmry_complm_document_type'
                                            );
                                            id_date = pymntCompDoc[id_invoice].trandate;
                                            id_type = pymntCompDoc[id_invoice].type;
                                            id_type = id_type.toLowerCase();
                                            id_amountpaid = pymntCompFields[i].getValue(
                                                'custrecord_lmry_complm_totalcommis_lc'
                                            );
                                            id_status = pymntCompDoc[id_invoice].status;
                                            id_status = id_status.toLowerCase();
                                            id_memo = pymntCompDoc[id_invoice].memomain;
                                            id_total = pymntCompFields[i].getValue(
                                                'custrecord_lmry_complm_totalcommis_lc'
                                            );
                                        }
                                    } else {
                                        ...
                                        id_doc = result_invoice2[i].getText(
                                            'custbody_lmry_document_type'
                                        );
                                        id_doc_value = result_invoice2[i].getValue(
                                            'custbody_lmry_document_type'
                                        );
                                        ...
                                        id_process_transact = result_invoice2[i].getValue(
                                            'custbody_lmry_processed_transaction'
                                        );
                                    }
                                    ...
                                    if (
                                        status_countries.indexOf(json_response[id_invoice][0]) != -1
                                    ) {
                                        continue;
                                    }
                                    ...
                                    if (featureAdvanced) {
                                        if (id_doc != null && id_doc != '') {
                                            ...
                                            if (
                                                json_document[id_doc_value] == undefined ||
                                                json_document[id_doc_value] == null
                                            ) {
                                                continue;
                                            }
                                            ...
                                            if (Rd_Country != 'BRA') {
                                                if (
                                                    jsonPackage[id_doc_value] == null ||
                                                    jsonPackage[id_doc_value] == undefined
                                                ) {
                                                    continue;
                                                }
                                            } else if (
                                                    jsonPackage[id_doc_value + ';' + id_type] == null ||
                                                    jsonPackage[id_doc_value + ';' + id_type] == undefined
                                                ) {
                                                    continue;
                                                }
                                        }
                ...
                form.clientScriptModulePath
                    = './EI_Library/LMRY_Invoicing_Populate_CLNT.js';
                ...
                if (p_state == null || p_state == '') {
                    redirect.toSuitelet({
                        ...
                        parameters: {
                            ...
                            custparam_transaction:
                                context.request.parameters.custpage_transaction,
                            custparam_checkpaid:
                                context.request.parameters.custpage_checkpaid
                        }
                    });
                } else {
                    ...
                    if (transaction == 'vendorbill' || transaction == 'vendorcredit') {
                        params.custscript_lmry_ip_params_state_purchase
                            = context.request.parameters.custpage_state;
                        params.custscript_lmry_ip_params_user_purchase = usuario;
                    } else if (
                        transaction == 'itemfulfillment' ||
                        transaccion == 'itemreceipt'
                    ) {
                        params.custscript_lmry_ip_params_state_fillrcpt
                            = context.request.parameters.custpage_state;
                        params.custscript_lmry_ip_params_user_fillrcpt = usuario;
                    } else {
                        params.custscript_lmry_ip_params_state
                            = context.request.parameters.custpage_state;
                    ...
                    switch (country2) {
                        ...
                        case 'ARG':
                            ...
                            if (libraryFeature.getAuthorization(899, licenses)) {
                                var deploySubsidiary
                                    = 'customdeploy_lmry_invoicing_pop_ar_' + subsidiary_id;
                                redMPRD = task.create({
                                    taskType: setting[0],
                                    scriptId: setting[1],
                        ...
                        case 'BOL':
                            ...
                            if (libraryFeature.getAuthorization(900, licenses)) {
                                var deploySubsidiary
                                    = 'customdeploy_lmry_invoicing_pop_bo_' + subsidiary_id;
                                redMPRD = task.create({
                                    taskType: setting[0],
                                    scriptId: setting[1],
                                    deploymentId: deploySubsidiary,
                        ...
                        case 'BRA':
                            ...
                            if (
                                ...
                                if (libraryFeature.getAuthorization(878, licenses)) {
                                    var deploySubsidiary
                                        = 'customdeploy_lmry_purchasing_pop_br_' + subsidiary_id;
                                    redMPRD = task.create({
                                        taskType: setting[0],
                                        scriptId: setting[2],
                                        deploymentId: deploySubsidiary,
                                ...
                                if (libraryFeature.getAuthorization(878, licenses)) {
                                    var deploySubsidiary
                                        = 'customdeploy_lmry_fillrcpt_pop_br_' + subsidiary_id;
                                    redMPRD = task.create({
                                        taskType: setting[0],
                                        scriptId: setting[3],
                                        deploymentId: deploySubsidiary,
                            ...
                            // Ventas
                                ...
                                if (libraryFeature.getAuthorization(878, licenses)) {
                                    var deploySubsidiary
                                        = 'customdeploy_lmry_invoicing_pop_br_' + subsidiary_id;
                                    redMPRD = task.create({
                                        taskType: setting[0],
                                        scriptId: setting[1],
                                        deploymentId: deploySubsidiary,
                        ...
                        case 'CHI':
                            if (transaction == 'itemfulfillment') {
                                if (libraryFeature.getAuthorization(901, licenses)) {
                                    var deploySubsidiary
                                        = 'customdeploy_lmry_fillrcpt_pop_ch_' + subsidiary_id;
                                    redMPRD = task.create({
                                        taskType: setting[0],
                                        scriptId: setting[3],
                                        deploymentId: deploySubsidiary,
                            ...
                            // Ventas
                                ...
                                if (libraryFeature.getAuthorization(901, licenses)) {
                                    var deploySubsidiary
                                        = 'customdeploy_lmry_invoicing_pop_ch_' + subsidiary_id;
                                    redMPRD = task.create({
                                        taskType: setting[0],
                                        scriptId: setting[1],
                                        deploymentId: deploySubsidiary,
                        ...
                        case 'COL':
                            ...
                            if (libraryFeature.getAuthorization(902, licenses)) {
                                var deploySubsidiary
                                    = 'customdeploy_lmry_invoicing_pop_co_' + subsidiary_id;
                                redMPRD = task.create({
                                    taskType: setting[0],
                                    scriptId: setting[1],
                                    deploymentId: deploySubsidiary,
                        ...
                        case 'MEX':
                            if (transaction == 'itemfulfillment') {
                                if (libraryFeature.getAuthorization(907, licenses)) {
                                    var deploySubsidiary
                                        = 'customdeploy_lmry_fillrcpt_pop_mx_' + subsidiary_id;
                                    redMPRD = task.create({
                                        taskType: setting[0],
                                        scriptId: setting[3],
                                        deploymentId: deploySubsidiary,
                            ...
                            // Ventas
                                ...
                                if (libraryFeature.getAuthorization(907, licenses)) {
                                    var deploySubsidiary
                                        = 'customdeploy_lmry_invoicing_pop_mx_' + subsidiary_id;
                                    redMPRD = task.create({
                                        taskType: setting[0],
                                        scriptId: setting[1],
                                        deploymentId: deploySubsidiary,
                        ...
                        case 'PER':
                            if (transaction == 'itemfulfillment') {
                                ...
                                if (libraryFeature.getAuthorization(911, licenses)) {
                                    var deploySubsidiary
                                        = 'customdeploy_lmry_fillrcpt_pop_pe_' + subsidiary_id;
                                    redMPRD = task.create({
                                        taskType: setting[0],
                                        scriptId: setting[3],
                                        deploymentId: deploySubsidiary,
                            ...
                            // Ventas
                                ...
                                if (libraryFeature.getAuthorization(911, licenses)) {
                                    var deploySubsidiary
                                        = 'customdeploy_lmry_invoicing_pop_pe_' + subsidiary_id;
                                    redMPRD = task.create({
                                        taskType: setting[0],
                                        scriptId: setting[1],
                                        deploymentId: deploySubsidiary,
                        ...
                        case 'PAN':
                            ...
                            if (libraryFeature.getAuthorization(909, licenses)) {
                                var deploySubsidiary
                                    = 'customdeploy_lmry_invoicing_pop_pa_' + subsidiary_id;
                                redMPRD = task.create({
                                    taskType: setting[0],
                                    scriptId: setting[1],
                                    deploymentId: deploySubsidiary,
                        ...
                        case 'COS':
                            ...
                            if (libraryFeature.getAuthorization(903, licenses)) {
                                var deploySubsidiary
                                    = 'customdeploy_lmry_invoicing_pop_cr_' + subsidiary_id;
                                redMPRD = task.create({
                                    taskType: setting[0],
                                    scriptId: setting[1],
                                    deploymentId: deploySubsidiary,
                        ...
                        case 'ECU':
                            ...
                            if (libraryFeature.getAuthorization(904, licenses)) {
                                var deploySubsidiary
                                    = 'customdeploy_lmry_invoicing_pop_ec_' + subsidiary_id;
                                redMPRD = task.create({
                                    taskType: setting[0],
                                    scriptId: setting[1],
                                    deploymentId: deploySubsidiary,
                        ...
                        case 'URU':
                            ...
                            if (libraryFeature.getAuthorization(913, licenses)) {
                                var deploySubsidiary
                                    = 'customdeploy_lmry_invoicing_pop_uy_' + subsidiary_id;
                                redMPRD = task.create({
                                    taskType: setting[0],
                                    scriptId: setting[1],
                                    deploymentId: deploySubsidiary,
                        ...
                        case 'GUA':
                            ...
                            if (libraryFeature.getAuthorization(906, licenses)) {
                                var deploySubsidiary
                                    = 'customdeploy_lmry_invoicing_pop_gt_' + subsidiary_id;
                                redMPRD = task.create({
                                    taskType: setting[0],
                                    scriptId: setting[1],
                                    deploymentId: deploySubsidiary,
                        ...
                        case 'PAR':
                            ...
                            if (libraryFeature.getAuthorization(910, licenses)) {
                                var deploySubsidiary
                                    = 'customdeploy_lmry_invoicing_pop_py_' + subsidiary_id;
                                redMPRD = task.create({
                                    taskType: setting[0],
                                    scriptId: setting[1],
                                    deploymentId: deploySubsidiary,
            ...
            strhtml
                += '<br>' + jsonLanguage.code[Language] + xml.escape(msgerr.name);
            strhtml
                += '<br>' + jsonLanguage.details[Language] + xml.escape(msgerr.message);
            strhtml += '</div>';
            strhtml += '</td>';
            strhtml += '</tr>';
            strhtml += '</table>';
    ...
    function advanced(licenses, country, idFeature) {
        ...
        return libraryFeature.getAuthorization(
            idFeature[country3country2[country]],
            licenses
        );
    }
    ...
    function validarAcentos(s) {
        var AccChars
            = 'ŠŽšžŸÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖÙÚÛÜÝàáâãäåçèéêëìíîïðñòóôõöùúûüýÿ&°–—ªº·';
        var RegChars
            = 'SZszYAAAAAACEEEEIIIIDNOOOOOUUUUYaaaaaaceeeeiiiidnooooouuuuyyyo--ao.';
    ...
    function status_invoice(country, arregloInvoice, subsidiary) {
        ...
        for (var j = 0; j < arregloInvoice.length; j++) {
            if (
                json_full[arregloInvoice[j]] == null ||
                json_full[arregloInvoice[j]] == undefined
            ) {
        ...
        if (country == 'MEX') {
            ...
            if (result_custbody != null && result_custbody.length > 0) {
                for (var i = 0; i < result_custbody.length; i++) {
                    json_full[result_custbody[i].getValue('internalid')][0]
                        = result_custbody[i].getValue(columnas[posicion]);
                }
            }
        } else if (country == 'PER') {
            var search_custbody = search.create({
            ...
            if (result_r_status != null && result_r_status.length > 0) {
                for (var i = 0; i < result_r_status.length; i++) {
                    json_full[
                        result_r_status[i].getValue('custrecord_lmry_ei_ds_doc')
                    ][0] = result_r_status[i].getValue(
                        'custrecord_lmry_ei_ds_doc_status'
                    );
                }
            }
