...
function(
            ...
            if (type_event == scriptContext.UserEventType.VIEW && type_interface == 'USERINTERFACE') {
                ...
                if (doc_country == 'MX' && (currentRecord.type == 'invoice' || currentRecord.type == 'creditmemo')) {
                    ...
                    if (currentRecord.type == 'customtransaction_lmry_payment_complemnt') {
                        ...
                        var count_invoice = currentRecord.getLineCount({
                            sublistId: 'apply'
                        });
                    }
    ...
    function removeButtonEdit(RCD_OBJ, OBJ_FORM, _docSubsi, _arrLicen) {
        try {
            ...
            if (['invoice', 'creditmemo', 'customerpayment', 'vendorcredit', 'itemfulfillment', 'customtransaction_lmry_payment_complemnt', 'customerdeposit'].indexOf(typeTransaction) == -1 || ['MX', 'BR', 'PE', 'CO'].indexOf(objCountry.code) == -1 || ((typeTransaction == 'vendorcredit' || typeTransaction == 'itemfulfillment') && objCountry.code != 'BR') || ((typeTransaction == 'customerpayment' || typeTransaction == 'customerdeposit') && objCountry.code != 'MX') || (objCountry.code == 'CO' && (_userObj.role == 3 || _userObj.role != 3 && !getAuthTxnVal(_arrLicen, '569')))) {
                return false;
            }
            ...
            if ((typeTransaction == 'creditmemo' && memo == 'Reference VOID' && objCountry.code != 'CO') || (objCountry.code == 'CO' && eiDocumentStatus.toLowerCase() == 'autorizado')) {
                hideButtons(OBJ_FORM);
            } else if (id_transaccion && objCountry.code != 'CO') {
    ...
    function sendEmailCustomer(currentRecord, form) {
        ...
        try {
            ...
            if (valSendemailStandar(currentRecord) && (currentRecord.type == 'invoice' || currentRecord.type == 'creditmemo' || currentRecord.type == 'customerpayment' || currentRecord.type == 'vendorbill' || currentRecord.type == 'itemfulfillment' || currentRecord.type == 'customtransaction_lmry_payment_complemnt' || currentRecord.type == 'customerdeposit')) {
                ...
                if ((par_country != '' && par_country != null) && (par_doctype != '' && par_doctype != null)) {
                    ...
                    if (getAuthTxnVal(all_Licenses, FEATURE_SENDCUSTOMER)) {
                        ...
                        if (valSendCustomer(par_country, currentRecord.id) && tieneContactos(par_customer, currentRecord.type)) {
    ...
    function tieneContactos(customerID, idtypetran) {
