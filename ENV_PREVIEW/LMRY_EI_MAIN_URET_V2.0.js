...
define([
    ...
    function (
        ...
        function getDeploy(recordObj, country) {
            ...
            filtros.push(search.createFilter({ name: 'custrecord_lmry_auto_inv_subsidiary_2', operator: 'is', values: recordObj.getValue('subsidiary') }));
            filtros.push(search.createFilter({ name: 'custrecord_lmry_auto_inv_document_2', operator: 'is', values: recordObj.getValue('custbody_lmry_document_type') }));
            if (country != '45') {
                filtros.push(search.createFilter({ name: 'custrecord_lmry_auto_inv_serie_2', operator: 'is', values: recordObj.getValue('custbody_lmry_serie_doc_cxc') }));
            }
            var searchDeploy = search.create({
                type: 'customrecord_lmry_auto_inv_multithr_2',
                filters: filtros,
                columns: ['custrecord_lmry_auto_inv_deploy_2']
            });
            searchDeploy = searchDeploy.run().getRange(0, 1);
            if (searchDeploy && searchDeploy.length) {
                nameDeploy = searchDeploy[0].getValue('custrecord_lmry_auto_inv_deploy_2');
            }
            return nameDeploy;
        ...
        function createLog(recordObj, nameDeploy) {
            ...
            var idLog = record.create({ type: 'customrecord_lmry_auto_inv_log_2' });
            idLog.setValue('custrecord_lmry_log_subsidiary_2', recordObj.getValue('subsidiary'));
            idLog.setValue('custrecord_lmry_log_transaction_2', recordObj.id);
            idLog.setValue('custrecord_lmry_log_user_2', currentUser);
            idLog.setValue('custrecord_lmry_log_status_2', '4');
            idLog.setValue('custrecord_lmry_log_deploy_2', nameDeploy);
            idLog.setValue('custrecord_lmry_log_transaction_type_2', recordObj.getValue('ntype'));
            idLog = idLog.save();
        }
        ...
        function getAutoEiAuthorization(form,currentRecord){
            var autoEiLicenses={'BR':1084,
                                'MX':1083,
                                'AR':1086,
                                'CL':1087};
            var validTransactions={'BR':['invoice'],
