 ...
 * @NModuleScope Public
 */
define(['N/record', 'N/currency', 'N/log', 'N/xml', 'N/ui/serverWidget', 'N/search', 'N/runtime', 'N/redirect', 'N/suiteAppInfo', 'N/format', 'N/config',
  './WTH_Library/LMRY_WHT_ON_Payments_LBRY', './Latam_Library/LMRY_libSendingEmailsLBRY_V2.0', "./Latam_Library/LMRY_Log_LBRY_V2.0"],

  function (record, currency, log, xml, serverWidget, search, runtime, redirect, suiteAppInfo, format, config, library, librarySend, lbryLog) {
    // Nombre del Script
    ...
    function onRequest(context) {
      try {
        if (context.request.method == 'GET') {
          ...
          if ((Number(Rd_SubId) && subsi_OW) || (Number(Rd_PayId) && !subsi_OW)) {
            var filtros = [];
            ...
            var search_setup = search.create({
              ...
              columns: ['custrecord_lmry_setuptax_check', 'custrecord_lmry_setuptax_prefix', 'custrecord_lmry_setuptax_minimum_digits',
                'custrecord_lmry_setuptax_current_number', 'custrecord_lmry_setuptax_elec_payment', 'custrecord_lmry_setuptax_currency', 'custrecord_lmry_setuptax_no_preview_pay']
            });
            ...
            if (result_search_setup && result_search_setup.length) {
              setupTaxSubsiary = {
                ...
                no_preview: result_search_setup[0].getValue({ name: 'custrecord_lmry_setuptax_no_preview_pay' })
              }
            }
          }
          ...
          if (librarySend.getAuthorization(1030, licenses)) {
            ...
            const filters = [];
            const countrySymbol = config.load({
                type: config.Type.COMPANY_INFORMATION
            }).getValue('country');
            filters.push(['custrecord_lmry_setuptax_sub_country','anyof', countrySymbol]);
            if (subsi_OW){
                if (Number(Rd_SubId)) {
          ...
          if (setupTaxSubsiary && setupTaxSubsiary.no_preview) {
          ...
          if (_estado == '' || _estado == null) {
            ...
            redirect.toSuitelet({
              ...
              parameters: {
                ...
                // "custparam_no_preview": context.request.parameters.custpage_no_preview
              }
            });
            ...
            if (dataLog) {
              ...
              if (logIds.length == 1) {
                redirect.toSuitelet({
                  ...
                  parameters: { param_logid: logIds[0], param_useid: usuario, param_nopreview: noPreview },
                });
              }
              if (logIds.length > 1) {
                redirect.toSuitelet({
                  ...
                  parameters: { param_logid: logIds.join(","), param_useid: usuario, param_nopreview: noPreview },
                });
              }
            }
