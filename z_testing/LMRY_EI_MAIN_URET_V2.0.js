        ...
        function beforeLoad(scriptContext) {
            try {
                ...
                if (type_event == scriptContext.UserEventType.CREATE || type_event == scriptContext.UserEventType.EDIT || type_event == scriptContext.UserEventType.COPY) {
                    ...
                    if (doc_country == "CL" && featureRoundedDecimal && transactionTypes.indexOf(currentRecord.type)!=-1) {
                        roundedDecimalUnitPrice(currentRecord);
                    }
            ...
            try {
                ...
                if (numberItems) {
                    for (var i = 0; i < numberItems; i++) {
                        var amount = currentRecord.getSublistValue({ sublistId: "item", fieldId: fieldid, line: i });
                        var roundedAmount = (Math.round(amount * 10) / 10).toFixed(2);
