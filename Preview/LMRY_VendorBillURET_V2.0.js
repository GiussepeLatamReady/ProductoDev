...
define(['require', './Latam_Library/LMRY_UniversalSetting_Purchase_LBRY', 'N/record', 'N/runtime', 'N/search', 'N/log', 'N/config', 'N/https', 'N/url', 'N/ui/serverWidget',
  ...
  function(require, library_Uni_Setting, record, runtime, search, log, config, https, url, serverWidget, library, library1, libraryDIOT, Library_WHT_Transaction, ST_Library_Transaction,
    ...
    function validateDeleteTaxResultesBR(context) {
      ...
      if (type === "edit") {
        ...
        if (validDelete) {
          ...
          results = taxResultSearch.run().getRange(0, 1);
        ...
        log.debug("validDelete", validDelete);
        if (validDelete) {
    ...
    function isEditAmounts(newRecord, oldRecord,context) {
      var state = false;
      if(context.type==='create'){
        log.error('Estado de la Transaccion1','En creacion')
        return true
      }
      if(!newRecord || !oldRecord){
        log.error('Estado de la Transaccion2','No comparable')
        return true
      };
      if(oldRecord.getValue('approvalstatus')!==newRecord.getValue('approvalstatus')){
        log.error('Estado de la Transaccion3','Estado de aprobacion modificado');
        return true;
      }
      ...
      if (nroItemsNew !== nroItemsOld){
        log.error('Estado de la Transaccion4','Modificada');
        return true;
      }
      ...
      if (nroExpenseNew !== nroExpenseOld){
        log.error('Estado de la Transaccion5','Modificada');
        return true;
      } 
      ...
      if(state===false){
        log.error('Estado de la Transaccion6','Sin modificacion');
      }else{
        log.error('Estado de la Transaccion7','Modificada');
      }
      return state
