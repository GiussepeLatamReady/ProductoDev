 ...
 */
 ...
 function(format, currencyApi, record, search, runtime, message, url, log, https, ei_library, library_AnulacionInvoice) {
         ...
         if (sublistId == 'item') {
           ...
           if (current_cfop != null && current_cfop != '') {
             ...
             if (resultCFOP != null && resultCFOP.length != 0) {
               ...
               log.error("cfop_value", cfop_value);
   ...
   function Remove_Trans(recordId, recordType) {
     ...
     if (recordType == 'customtransaction_lmry_payment_complemnt') {
       ...
       if (F_REVERSALVOIDING == true || F_REVERSALVOIDING == 'T') {
         var resultJournal = library_AnulacionInvoice.reversalJournal(rec_payment);
       ...
       if (flag) {
         ...
         if (Typevento == 'cancelacion') {
           if (objResponse.body == 'true') {
             if (docCountry == 'MX' && codModification != '01' && (sTypeRec == 'customtransaction_lmry_payment_complemnt' || sTypeRec == 'customerpayment')) {
               Remove_Trans(nInternalId, sTypeRec)
             }
             var myMsg = message.create({
   ...
   function Status_EI(internal_Id, id_subsi, rechazar) {
     try {
       ...
       if (flag && flagRellamadoConcurrente) {
         if(BR_EmptyFolioFiscal(internal_Id, id_subsi)){
          log.error('testfolio','entro');
         }
   ...
   function BR_EmptyFolioFiscal(internal_Id, id_subsi){
     var country = ei_library.getCountryID(id_subsi).code;
     ...
     log.error('TestAndy',folio+'-'+status);
     return (country == 'BR' && (folio == '' || folio == null) && status == 'Autorizado')
   }
   ...
   function block_lmry_cl_ei_exchangerate(currentRCD) {
     ...
     try {
       ...
       if (currentRCD.getValue('subsidiary') != '' && currentRCD.getValue('subsidiary') != null && currentRCD.getValue('currency') != '' && currentRCD.getValue('currency') != null) {
