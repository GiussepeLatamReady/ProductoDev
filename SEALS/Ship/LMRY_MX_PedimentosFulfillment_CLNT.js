...
define([
    ...
    function pageInit(scriptContext) {
        try {
            ...
            //Setea Fecha Actual
            if (
                RCD_OBJ.getValue({ fieldId: "custpage_id_date" }) == null ||
                RCD_OBJ.getValue({ fieldId: "custpage_id_date" }) == ""
            ) {
    ...
    function fieldChanged(scriptContext) {
        try {
            if (
                scriptContext.sublistId == "custpage_id_sublista" &&
                scriptContext.fieldId == "id_popup"
            ) {
                var RCD_OBJ = scriptContext.currentRecord;
                var item_id = RCD_OBJ.getCurrentSublistValue({
                    sublistId: "custpage_id_sublista",
                    fieldId: "id_item",
                });
                var location_id = RCD_OBJ.getCurrentSublistValue({
                    sublistId: "custpage_id_sublista",
                    fieldId: "id_location",
                });
                var serie_lote_id = RCD_OBJ.getCurrentSublistValue({
                    sublistId: "custpage_id_sublista",
                    fieldId: "custpage_lote",
                });
                var current_line = nlapiGetCurrentLineItemIndex("custpage_id_sublista");
                var descrip = nlapiGetLineItemValue(
                    "custpage_id_sublista",
                    "custpage_description",
                    current_line
                );
                var quantity = RCD_OBJ.getCurrentSublistValue({
                    sublistId: "custpage_id_sublista",
                    fieldId: "custpage_quantity",
                });
                var line_id = RCD_OBJ.getCurrentSublistValue({
                    sublistId: "custpage_id_sublista",
                    fieldId: "custpage_line_id",
                });
                ...
                RCD_OBJ.setCurrentSublistValue({
                    sublistId: "custpage_id_sublista",
                    fieldId: "id_popup",
                    value: true,
        ...
        try {
            ...
            if (cant > 0) {
                for (var i = 0; i < cant; i++) {
					var jsonPedimentos = [];
                    var s_intr = RCD_OBJ.getSublistValue({
                        sublistId: "custpage_id_sublista",
                        fieldId: "id_item",
                        line: i,
                    });
                    var s_location = RCD_OBJ.getSublistValue({
                        sublistId: "custpage_id_sublista",
                        fieldId: "id_location",
                        line: i,
                    });
                    var s_lote = RCD_OBJ.getSublistValue({
                        sublistId: "custpage_id_sublista",
                        fieldId: "custpage_lote",
                        line: i,
                    });
                    var s_detalle_ped = RCD_OBJ.getSublistValue({
                        sublistId: "custpage_id_sublista",
                        fieldId: "custpage_popup_data_detail",
                        line: i,
                    });
					console.log("s_detalle_ped", s_detalle_ped);
                    ...
                    if (Object.keys(jsonPedimentos).length) {
                        ...
                        alert(
                            "Debe llenar todas las líneas con su(s) número(s) de pedimento(s) correspondientes."
                        );
                        return false;
                    }
                }
                ...
                for (var i = 0; i < item_json.length; i++) {
                    ...
                    var ped_details = record.create({
                        type: "customrecord_lmry_mx_pedimento_details",
                        isDynamic: true,
                    });

                    ped_details.setValue({
                        fieldId: "custrecord_lmry_mx_ped_subsidiary",
                        value: _sub,
                    });
                    ped_details.setValue({
                        fieldId: "custrecord_lmry_mx_ped_trans_ref",
                        value: _source,
                    });
                    ped_details.setValue({
                        fieldId: "custrecord_lmry_source_transaction_id",
                        value: _source,
                    });
                    ped_details.setValue({
                        fieldId: "custrecord_lmry_mx_ped_trans",
                        value: _transac,
                    });
                    ped_details.setValue({
                        fieldId: "custrecord_lmry_transaction_id",
                        value: _transac,
                    });
                    ped_details.setValue({ fieldId: "custrecord_lmry_date", value: _date });
                    ped_details.setValue({
                        fieldId: "custrecord_lmry_mx_ped_item",
                        value: item_json[i].item,
                    });
                    ...
                    ped_details.setValue({
                        fieldId: "custrecord_lmry_mx_ped_num",
                        value: item_json[i].pediment,
                    });
                    ped_details.setValue({
                        fieldId: "custrecord_lmry_mx_ped_location",
                        value: item_json[i].location,
                    });
                    ped_details.setValue({
                        fieldId: "custrecord_lmry_mx_ped_quantity",
                        value: item_json[i].quantity,
                    });

                    var lote = item_json[i].lote;
                    if (lote != null && lote != "") {
                        ped_details.setValue({
                            fieldId: "custrecord_lmry_mx_ped_lote_serie",
                            value: item_json[i].lote,
                        });
                    }

                    if (item_json[i].aduana) {
                        ped_details.setValue({
                            fieldId: "custrecord_lmry_mx_ped_aduana",
                            value: item_json[i].aduana,
                        });
                    }
