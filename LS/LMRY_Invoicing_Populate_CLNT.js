    ...
    function (search, library, currentRecord, record, url, runtime, libraryFeature) {
        ...
        function pageInit(scriptContext) {
            ...
            try {
                ...
                if(!objRecord.getField('custpage_state')){
                    objRecord.getField('custpage_checkpaid').isVisible = false;
                    objRecord.getField('custpage_mx_payment_method').isVisible = false;
                }
                ...
                if (p_hidden == null || p_hidden == '') {
                    if (!objRecord.getValue('custpage_date') && !objRecord.getValue('custpage_ddate_from')) {
                        ...
                        objRecord.setValue('custpage_date', new Date());
                        objRecord.setValue('custpage_date_2', new Date());

                        /*objRecord.setValue('custpage_ddate_from', newDate);
                        objRecord.setValue('custpage_ddate_to', newDate);*/
        ...
        function validateField(scriptContext) {
            ...
            try {
                ...
                if (scriptContext.fieldId == 'custpage_transaction') {
                    ...
                    viewPaymentMethod(scriptContext);
                }
            } catch (error) {
        ...
        function viewPaymentMethod(scriptContext){

            var objRecord = scriptContext.currentRecord;
            var country = objRecord.getValue('custpage_country_id');
            var transaction = objRecord.getValue('custpage_transaction');
            transaction = transaction.split(';')[0];

            if(country == '157' && transaction == 'invoice'){
                objRecord.getField('custpage_mx_payment_method').isVisible = true;
            }else{
                objRecord.setValue('custpage_mx_payment_method', '');
                objRecord.getField('custpage_mx_payment_method').isVisible = false;
            }
        ...
        function saveRecord(scriptContext) {
            ...
            try {
                ...
                var date = objRecord.getValue('custpage_date');
                var date_2 = objRecord.getValue('custpage_date_2');
                ...
                if (state == null || state == '') {
                    ...
                    if (v_Country == 174) {
                        if (FEATURE_DEPT == true || FEATURE_DEPT == 'T') {
                            if (DEPTMANDATORY == 'T' || DEPTMANDATORY == true) {
                                var f_department = objRecord.getValue({
                                    fieldId: 'custpage_department'
                                });
                                if (f_department == null || f_department == '' || f_department == 0) {
                                    alert(jsonLanguage.department[Language]);
                                    return false;
                                }
                            }
                        }
                    ...
                    if (v_Country == 174) {
                        if (FEATURE_CLASS == true || FEATURE_CLASS == 'T') {
                            if (CLASMANDATORY == 'T' || CLASMANDATORY == true) {
                                var f_class = objRecord.getValue({
                                    fieldId: 'custpage_class'
                                });
                                if (f_class == null || f_class == '' || f_class == 0) {
                                    alert(jsonLanguage.class[Language]);
                                    return false;
                                }
                            }
                        }
                    ...
                    if (v_Country == 174) {
                        if (FEATURE_LOC == true || FEATURE_LOC == 'T') {
                            if (LOCMANDATORY == 'T' || LOCMANDATORY == true) {
                                var f_location = objRecord.getValue({
                                    fieldId: 'custpage_location'
                                });
                                if (f_location == null || f_location == '' || f_location == 0) {
                                    alert(jsonLanguage.location[Language]);
                                    return false;
                                }
                            }
                        }
        ...
        function mostrarCheckPaidInvoices(scriptContext) {
            ...
            if(transaction == 'invoice'){
                objRecord.getField('custpage_checkpaid').isVisible = true;
            }else{
                objRecord.setValue('custpage_checkpaid', '1');
                objRecord.getField('custpage_checkpaid').isVisible = false;
            }
