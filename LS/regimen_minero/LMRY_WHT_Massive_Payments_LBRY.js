 ...
 * @NModuleScope Public
 */
define(['N/ui/serverWidget', 'N/search', 'N/record', 'N/log', 'N/url', 'N/format', 'N/runtime', '/SuiteBundles/Bundle 37714/Latam_Library/LMRY_libSendingEmailsLBRY_V2.0', "/SuiteBundles/Bundle 37714/Latam_Library/LMRY_Log_LBRY_V2.0", "N/query"],
  ...
  function (serverWidget, search, record, log, url, format, runtime, library_send, lbryLog, query) {
    ...
    //Regimen Minero
    var jsonRG = {}, jsonSalta = {}, jsonTaxGroups = {};
    var jsonFSalta = {}, jsonFRG = {};
    var jsonAcumSalta = {}, jsonAcumRG = {};
    ...
    function payment_log(script, idLog, accountingPeriod, vendor, fechaIngresada, currencies, result_setuptax, subsidiary, docfiscal_id, docfiscal_bill, exchange_global, noretention, manual, isMultiCuenta) {
      ...
      log.debug('ST_FEATURE', ST_FEATURE);
      ...
      if (result_log != null && result_log.length > 0) {
        for (var i = 0; i < result_log.length; i++) {
          ...
          //Acumulados por Conceptos Ganancias
          var acumRG = result_log[i].custrecord_ls_bases_ganancias || "{}";
          acumRG = JSON.parse(acumRG);

          for(var j in acumRG){
            jsonAcumRG[j] = jsonAcumRG[j] || {};
            for(var k in acumRG[j]){
              jsonAcumRG[j][k] = jsonAcumRG[j][k] || 0;
              jsonAcumRG[j][k] += parseFloat(acumRG[j][k]);
            }
          }

          //Acumulados por Conceptos Salta
          var acumSalta = result_log[i].custrecord_ls_bases_ingresos_brutos || "{}";
          acumSalta = JSON.parse(acumSalta);
          
          for(var j in acumSalta){
            jsonAcumSalta[j] = jsonAcumSalta[j] || {};
            for(var k in acumSalta[j]){
              jsonAcumSalta[j][k] = jsonAcumSalta[j][k] || 0;
              jsonAcumSalta[j][k] += parseFloat(acumSalta[j][k]);
            }
          }
      ...
      log.debug('jsonAcumRG', jsonAcumRG);
      log.debug('jsonAcumSalta', jsonAcumSalta);

      //NOTA DE CREDITO
      var current_log = search.lookupFields({
        type: 'customrecord_lmry_wht_payments_log',
        id: idLog,
        columns: ['custrecord_lmry_wht_bil', 'custrecord_lmry_wht_ven', 'custrecord_lmry_wht_cre', 'custrecord_ls_assign_concepts']
      });
      ...
      var assignConcepts = current_log.custrecord_ls_assign_concepts;

      log.debug('assignConcepts', assignConcepts);
      ...
      //log.debug('lista_bills_pagar', lista_bills_pagar);

      regimenMinero(lista_bills_pagar, json_auxiliar, exchange_global, auxCredit, assignConcepts);
      ...
      log.debug("jsonRaCC", JSON.stringify(jsonRaCC));
      log.debug("jsonRaNt", JSON.stringify(jsonRaNT));
        ...
        if (recorpay != null && recorpay != '') {
          var id = record.submitFields({
            ...
            values: {
              ...
              custrecord_ls_bases_ganancias: JSON.stringify(jsonFRG),
              custrecord_ls_bases_ingresos_brutos: JSON.stringify(jsonFSalta)
            },
            options: {
    ...
    function iteracion(script, bySubsidiary, result, columnas, porcet, posicion, tramo, vendor, orden, docFiscal, exchange_global, manual, noretention, suma, vendorvalue, pagoDeBill, jsonRaCC, jsonRaNT, isMultiCuenta, whtGroupByVendorActive) {
      ...
      if (result != null && result.length > 0) {
        for (var k = 0; k < result.length; k++) {
          ...
          var saltaConcept = result[k][columnas[31]] || '';
          var rgConcept = result[k][columnas[30]] || '';
          var saltaConceptText = result[k][columnas[31] + "_text"] || '';
          var rgConceptText = result[k][columnas[30] + "_text"] || '';
          var defConcept = '';
          ...
          var suma_global = 0, tra_total_global = 0, amon_global = 0;

          //Definición de bases, acumulados
          if(appl_to == 5){
            //Concepto Ganancias
            ...
            if(!jsonRG[tramo[0]][rgConcept]){
              continue;
            }

            if(!jsonFRG[rgConcept]){
              continue;
            }

            if(!jsonFRG[rgConcept][String(taxcode_group)]){
              continue;
            }

            suma_global = jsonAcumRG[rgConcept] ? (jsonAcumRG[rgConcept][String(taxcode_group)] || 0) : 0;
            tra_total_global = jsonFRG[rgConcept][String(taxcode_group)];
            amon_global = jsonRG[tramo[0]][rgConcept][String(taxcode_group)];

            defConcept = rgConceptText;

          }else if(appl_to == 6){
            //Actividad Enconómica Salta
            if(!saltaConcept){
              continue;
            }

            if(!jsonSalta[tramo[0]]){
              continue;
            }

            if(!jsonSalta[tramo[0]][saltaConcept]){
              continue;
            }

            if(!jsonFSalta[saltaConcept]){
              continue;
            }

            if(!jsonFSalta[saltaConcept][String(taxcode_group)]){
              continue;
            }

            suma_global = jsonAcumSalta[saltaConcept] ? (jsonAcumSalta[saltaConcept][String(taxcode_group)] || 0) : 0;
            tra_total_global = jsonFSalta[saltaConcept][String(taxcode_group)];
            amon_global = jsonSalta[tramo[0]][saltaConcept][String(taxcode_group)];

            defConcept = saltaConceptText;

          }else{//Totales es 1, 2 lineas no soportado se tomará como totales

            suma_global = acumulados_taxgroup[0],
            tra_total_global = suma_taxgroup[0],
            amon_global = json_prueba.total_gravado[posicion];
          ...
          if (casoActual == "2") {
            ...
            if (val > 0) {
              ...
              arregloAuxiliar[contadorAuxiliar] += '|' + defConcept;
    ...
    function llenadoSubTabla(subTabla, proceso, arregloBanderas, subsidiary, language, isMultiCuenta) {
      ...
      //Nueva columna para el concepto
      subTabla.addField({
        id: 'sub_concept',
        label: LblConcept,
        type: serverWidget.FieldType.TEXT
      ...
      for (var i = 0; i < arregloAuxiliar.length; i++) {
        ...
        if(colFields[50]){
          subTabla.setSublistValue({id: 'sub_concept', line: i, value: colFields[50]});
        }
    ...
    function regimenMinero(idBills, jsonPorcet, rateToPesos, auxCredit, assignConcepts){
      
      var credits = getCredits(auxCredit);
      getTaxCodeGroups();
      getBasesConceptos(idBills, rateToPesos, credits, assignConcepts);
      getCurrencyBase(jsonSalta, jsonFSalta, jsonPorcet);
      getCurrencyBase(jsonRG, jsonFRG, jsonPorcet);

      log.debug('jsonFSalta', jsonFSalta);
      log.debug('jsonFRG', jsonFRG);

    }

    function getCredits(auxCredit){

      var idCredits = [];

      if(auxCredit){
        auxCredit = JSON.parse(auxCredit);

        for(var i in auxCredit){
          for(var j in auxCredit[i]){
            idCredits.push(j);
          }
      ...
      //log.debug('idCredits', idCredits);

      return idCredits;

    }

    function getTaxCodeGroups(){

      var searchTaxGroups = search.create({
        type: 'customrecord_lmry_setup_taxcode_group',
        columns: ['custrecord_lmry_setup_taxcode', 'custrecord_lmry_setup_taxcode_group', {name: 'custrecord_lmry_setup_amount_to', join: 'custrecord_lmry_setup_taxcode_group'}],
        filters: [
          {name: 'custrecord_lmry_setup_taxcode_country', operator: 'is', values: 11},
          {name: 'isinactive', operator: 'is', values: 'F'}
        ]
      });

      searchTaxGroups = searchTaxGroups.run().getRange({start: 0, end: 1000});

      if(searchTaxGroups && searchTaxGroups.length){
        for(var i = 0; i < searchTaxGroups.length; i++){
          var idTaxCodeGroup = searchTaxGroups[i].getValue('custrecord_lmry_setup_taxcode_group');
          var idTaxCode = searchTaxGroups[i].getValue('custrecord_lmry_setup_taxcode');
          var baseTaxCode = searchTaxGroups[i].getValue({name: 'custrecord_lmry_setup_amount_to', join: 'custrecord_lmry_setup_taxcode_group'});

          jsonTaxGroups[idTaxCodeGroup] = jsonTaxGroups[idTaxCodeGroup] || {};
          jsonTaxGroups[idTaxCodeGroup]['base'] = baseTaxCode;
          jsonTaxGroups[idTaxCodeGroup]['taxcode'] = jsonTaxGroups[idTaxCodeGroup]['taxcode'] || {};
          jsonTaxGroups[idTaxCodeGroup]['taxcode'][idTaxCode] = idTaxCode;

        }
      }

      log.debug('jsonTaxGroups', jsonTaxGroups);

      //return jsonTaxGroups;

    }

    function getBasesConceptos(idBills, rateToPesos, credits, assignConcepts){

      if(assignConcepts){
        assignConcepts = JSON.parse(assignConcepts);
      }

      var idBillCredits = [];
      for(var i = 0; i < idBills.length; i++){
        idBillCredits.push(idBills[i]);
      }

      if(credits.length){
        for(var i = 0; i < credits.length; i++){
          idBillCredits.push(credits[i]);
        } 
      }
      ...
      if(credits.length){
        ...
        var searchApplied = search.create({
          type: 'vendorcredit',
          filters: [
            ["internalid", "anyof", credits], "AND",
            ["mainline","is","T"]
          ],
          columns: ["appliedtotransaction", {name: 'internalid', join: 'appliedToTransaction'}]
        });
  
        searchApplied = searchApplied.run().getRange({start: 0, end: 1000});
  
        for(var i = 0; i < searchApplied.length; i++){
          var idApplied = searchApplied[i].getValue('appliedtotransaction');
          jsonApplied[searchApplied[i].id] = idApplied;
      ...
      var searchBills = search.create({
        type: 'transaction',
        filters: [
          ["type","anyof","VendBill","VendCred"], "AND",
          ["internalid", "anyof", idBillCredits], "AND",
          ["mainline","is","F"], "AND",
          [[["item.custitem_ls_ar_item_concept_salta","noneof","@NONE@"]],"OR",[["item.custitem_ls_ar_item_concept_rg5333","noneof","@NONE@"]]]
        ],
        columns: ['item', {name: 'internalid', join: 'taxItem'}, 'fxamount', {name: 'formulacurrency', formula: '{fxamount} * ({taxitem.rate} / 100)'}, {name: 'formulacurrency', formula: '{fxamount} * (({taxitem.rate} / 100) + 1)'},
          {name: 'custitem_ls_ar_item_concept_salta', join: 'item'}, {name: 'custitem_ls_ar_item_concept_rg5333', join: 'item'}, "type", {name: 'internalid', join: 'appliedToTransaction'}, "lineuniquekey"]
      });

      searchBills = searchBills.run().getRange({start: 0, end: 1000});

      if(searchBills && searchBills.length){
        var columnBill = searchBills[0].columns;
        for(var i = 0; i < searchBills.length; i++){

          
          var idItem =  searchBills[i].getValue('item');
          var typeTransaction = searchBills[i].getValue('type');
          var appliedTo = searchBills[i].getValue({name: 'internalid', join: 'appliedToTransaction'});
          var taxcodeLinea = searchBills[i].getValue(columnBill[1]);

          if(typeTransaction == 'VendBill'){

            var idBill = searchBills[i].id;
            var netoLinea = searchBills[i].getValue(columnBill[2]);
            var taxLinea = searchBills[i].getValue(columnBill[3]) || 0;
            var grossLinea = searchBills[i].getValue(columnBill[4]);

          }else{

            var idBill = appliedTo;

            if(!idBill){
              idBill = jsonApplied[searchBills[i].id];
            }

            var netoLinea = parseFloat(searchBills[i].getValue(columnBill[2]));
            var taxLinea = searchBills[i].getValue(columnBill[3]) || 0;
            taxLinea = parseFloat(taxLinea);
            var grossLinea = parseFloat(searchBills[i].getValue(columnBill[4]));
          }
          
          netoLinea = round2(netoLinea);
          taxLinea = round2(taxLinea);
          grossLinea = round2(grossLinea);        

          var taxcodeLinea = searchBills[i].getValue(columnBill[1]);
          var conceptoSalta = searchBills[i].getValue(columnBill[5]) || '';
          var conceptoRG = searchBills[i].getValue(columnBill[6]) || '';

          if(assignConcepts){
            if(assignConcepts[idBill] && assignConcepts[idBill][idItem]){

              conceptoSalta = assignConcepts[idBill][idItem]['newSalta'] || '';
              conceptoRG = assignConcepts[idBill][idItem]['newGanancias'] || '';

            }
          }

          if(conceptoSalta){
            jsonSalta[idBill] = jsonSalta[idBill] || {};
            jsonSalta[idBill][conceptoSalta] = jsonSalta[idBill][conceptoSalta] || {};
          }

          if(conceptoRG){
            jsonRG[idBill] = jsonRG[idBill] || {};
            jsonRG[idBill][conceptoRG] = jsonRG[idBill][conceptoRG] || {};
          }

          for(var j in jsonTaxGroups){

            if(!jsonTaxGroups[j]['taxcode'][taxcodeLinea]){
              continue;
            }

            var baseTaxCode = jsonTaxGroups[j]['base'];
            baseTaxCode = getBaseTaxCode(baseTaxCode, grossLinea, taxLinea, netoLinea);

            if(conceptoSalta){
              jsonSalta[idBill][conceptoSalta][j] = jsonSalta[idBill][conceptoSalta][j] || 0;
              jsonSalta[idBill][conceptoSalta][j] += round4(parseFloat(baseTaxCode) / parseFloat(rateToPesos));
              jsonSalta[idBill][conceptoSalta][j] = round4(jsonSalta[idBill][conceptoSalta][j]);
            }

            if(conceptoRG){
              jsonRG[idBill][conceptoRG][j] = jsonRG[idBill][conceptoRG][j] || 0;
              jsonRG[idBill][conceptoRG][j] += round4(parseFloat(baseTaxCode) / parseFloat(rateToPesos));
              jsonRG[idBill][conceptoRG][j] = round4(jsonRG[idBill][conceptoRG][j]);
      ...
      log.debug('jsonSalta', jsonSalta);
      log.debug('jsonRG', jsonRG);


    }

    function getBaseTaxCode(baseTaxCode, grossLinea, taxLinea, netoLinea){

      if(baseTaxCode == 1){
        return grossLinea;
      }else if(baseTaxCode == 2){
        return taxLinea;
      }else if(baseTaxCode == 3){
        return netoLinea;
      }

    }

    //Prorratea dado pagos parciales (jsonPorcet)
    function getCurrencyBase(jsonByBill, jsonFull, jsonPorcet){

      for(var i in jsonByBill){
        for(var j in jsonByBill[i]){
          jsonFull[j] = jsonFull[j] || {};

          for(var k in jsonByBill[i][j]){
            jsonFull[j][k] = jsonFull[j][k] || 0;

            var baseConcepto = jsonByBill[i][j][k] * parseFloat(jsonPorcet[i]);
            baseConcepto = round4(baseConcepto);

            //baseConcepto = baseConcepto / parseFloat(rateToPesos);
            //baseConcepto = round4(baseConcepto);

            jsonFull[j][k] += baseConcepto;

            jsonFull[j][k] = round4(jsonFull[j][k]);
