...
define(['N/log', 'N/xml', 'N/format', 'N/config', 'N/redirect', 'N/url', 'N/task', 'N/search', 'N/record', 'N/ui/serverWidget',
    ...
    function (log, xml, format, config, redirect, url, task, search, recordApi, serverWidget, runtime, library, libraryFeature, http, https) {
        ...
        var jsonLanguage = {
            ...
            paymentmethod: {
            en: 'LatamReady - MX Payment Method',
            es: 'LatamReady - MX Método Pago',
            pt: 'LatamReady - MX Método de Pagamento',
            },
            unpaid: {
        ...
        function onRequest(context) {
            try {
                ...
                if (context.request.method == 'GET') {
                    ...
                    var Rd_PaymentMethod = context.request.parameters.custparam_paymentmethod;
                    ...
                    var fPaymentMethod = form.addField({id: 'custpage_mx_payment_method', type: 'select', label: jsonLanguage['paymentmethod'][Language], container: 'group_pi'});

                    addPMOptions(fPaymentMethod);
                    ...
                    let fDDateFrom = form.addField({id: 'custpage_ddate_from', type: 'date', label: jsonLanguage.ddatesfrom[Language], container: 'group_ddates', source: 'date'}).updateDisplaySize({height: 60, width: 30});
                    let fDDateTo = form.addField({id: 'custpage_ddate_to', type: 'date', label: jsonLanguage.ddatesto[Language], container: 'group_ddates', source: 'date'}).updateDisplaySize({height: 60, width: 30});
                    ...
                    if (Rd_Country) {
                        ...
                        if(Rd_PaymentMethod){
                        filtros_invoice.push(search.createFilter({name: 'custbody_lmry_mx_paymentmethod', operator: 'is', values: Rd_PaymentMethod}));
                        }
                        ...
                        log.debug('result_invoice3', result_invoice3);
                        ...
                        fPaymentMethod.defaultValue = Rd_PaymentMethod;
                        fPaymentMethod.updateDisplayType({displayType: 'disabled'});
                        }else{
                            fPaymentMethod.updateDisplayType({displayType: 'hidden'});
                        }
                        ...
                        p_checkpaid.updateDisplayType({displayType: 'disabled'});
                        }else{
                            p_checkpaid.updateDisplayType({displayType: 'hidden'});
                        }
                    ...
                    if (p_state == null || p_state == '') {
                        redirect.toSuitelet({
                            ...
                            parameters: {
                                ...
                                custparam_paymentmethod: context.request.parameters.custpage_mx_payment_method,
        ...
        function addPMOptions(fPaymentMethod){

        var searchPM = search.create({
            type: 'customrecord_lmry_mx_payment_method',
            filters: [
                {name: 'isinactive', operator: 'is', values: 'F'}
            ],
            columns: ['custrecord_lmry_mx_code_payment_method']
        });

        searchPM = searchPM.run().getRange(0, 1000);

        if(searchPM && searchPM.length){

            fPaymentMethod.addSelectOption({value: '', text: '&nbsp;'});

            for(var i = 0; i < searchPM.length; i++){

                fPaymentMethod.addSelectOption({
                    value: searchPM[i].id,
                    text: searchPM[i].getValue('custrecord_lmry_mx_code_payment_method')
                });
