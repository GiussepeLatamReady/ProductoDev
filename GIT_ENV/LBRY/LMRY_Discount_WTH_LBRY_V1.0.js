...
* File : LMRY_Discount_WTH_LBRY_V1.0.js
*/
...
function discountWithHolding(transactionRecord, standardLines, customLines, book, licenses) {
  try {
    ...
    if (transactionType != 'vendorpayment' && transactionType != 'vendorprepaymentapplication') {
      return true;
    }
    ...
    if (resultSearchSub.length != null && resultSearchSub.length > 0) {
      ...
      var applySublist = (transactionType == 'vendorpayment') ? transactionRecord.getLineItemCount('apply') : transactionRecord.getLineItemCount('bill');
      var applySb = (transactionType == 'vendorpayment') ? 'apply' : 'bill';
      ...
      nlapiLogExecution('DEBUG', 'transaccionPagar', JSON.stringify(transaccionPagar));
      ...
      columnasMX[8] = new nlobjSearchColumn("custrecord_lmry_mx_diot_ret_iva_total");
        ...
        for (var i = 0; i < resultMX.length; i++) {
          ...
          if (!jsonMX[idBill]) {
            jsonMX[idBill] = {
              ...
              'ret': parseFloat(resultMX[i].getValue(columMX[8]))
            };
          } else {
            jsonMX[idBill]['ret'] += parseFloat(resultMX[i].getValue(columMX[8]));
          }

        }
      }

      if (!Object.keys(jsonMX).length) {
        return true;
      }
      ...
      if (featureMB) {
        ...
        //nlapiLogExecution('DEBUG','jsonGlobalAM',JSON.stringify(jsonGlobalAM));

      }

      //NUEVA BUSQUEDA DE BILL CREDITS APLICADOS (PARA CALCULAR LA RET PARCIAL [SI FUERA ASINCRONO NO SERIA NECESARIO])
      var jsonCredit = {};

      var filtrosCredit = [];
      filtrosCredit[0] = new nlobjSearchFilter("internalid", "appliedtotransaction", "anyof", transaccionPagar);
      filtrosCredit[1] = new nlobjSearchFilter("mainline", null, "is", 'T');
      filtrosCredit[2] = new nlobjSearchFilter("voided", null, "is", 'F');
      filtrosCredit[3] = new nlobjSearchFilter("posting", null, "is", 'T');
      filtrosCredit[4] = new nlobjSearchFilter("appliedtoforeignamount", null, "greaterthan", '0.00');

      var columnasCredit = [];
      columnasCredit[0] = new nlobjSearchColumn("internalid");
      columnasCredit[1] = new nlobjSearchColumn("appliedtoforeignamount");
      columnasCredit[2] = new nlobjSearchColumn("internalid", "appliedtotransaction");

      var searchCredit = nlapiCreateSearch("vendorcredit", filtrosCredit, columnasCredit);
      searchCredit = searchCredit.runSearch().getResults(0, 1000);

      if (searchCredit && searchCredit.length) {
        var columCredit = searchCredit[0].getAllColumns();

        for (var i = 0; i < searchCredit.length; i++) {
          var monto = Math.abs(parseFloat(searchCredit[i].getValue(columCredit[1])));
          var idBill = searchCredit[i].getValue(columCredit[2]);

          jsonCredit[idBill] = jsonCredit[idBill] || 0;
          jsonCredit[idBill] += parseFloat(monto);

        }

      }

      nlapiLogExecution('DEBUG', 'jsonCredit', JSON.stringify(jsonCredit));
      ...
      for (var i = 1; i <= applySublist; i++) {
        ...
        if (apply == 'T' || apply == true) {
          ...
          var retencion = jsonMX[lineID]['ret'];
          ...
          var amountIVA = parseFloat(linePaymentAmount) * parseFloat(percentaje); // IVA de la Transaccion

          if (jsonCredit[lineID]) {
            grossamt -= parseFloat(jsonCredit[lineID]);
          }
          retencion = parseFloat(retencion) * (parseFloat(linePaymentAmount) / parseFloat(grossamt));
          ...
          if (amountIVA == 0 || amountIVA == '' || amountIVA <= 0) {
          ...
          } else { // Bill Payment o Vendor Prepayment
    ...
    nlapiLogExecution('ERROR', '[ discountWithHolding ]', error);
    throw ' [ discountWithHolding ] ' + error;
  }
}
...
function getBookExchangeRate(transactionRecordAux, book, exchange) {
  ...
  if (book.isPrimary()) {
      ...
      if (transactionRecordAux.getRecordType().toLowerCase() == 'vendorprepaymentapplication') {
          ...
          // Versión fallida de encontrar exchangeRate en vendorprepaymentapplication - 1/4/2022
